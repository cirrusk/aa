...
<c:forEach var="row" items="${examPaperNonDegreeList}" varStatus="i">
	<!-- 		날짜 표시 세팅 -->
	...
	<!-- 	div박스 그리기시작 -->
	<div class="article sortable" type="7" date="${date}" id="nonDegreeExamPaper_<c:out value="${row.courseActiveExamPaper.courseActiveExamPaperSeq}"/>" >
		<h3><span class="test"></span><spring:message code="필드:시험:시험"/></h3>
		<c:choose>
			<c:when test="${possibleYn eq 'W'}">
			...
			<c:when test="${row.courseActiveExamPaper.openYn eq 'Y'}">
				<c:choose>
					<c:when test="${row.courseActiveExamPaper.scoredCount gt 0}">
						<a href="javascript:void(0)" class="thum active" onclick="doExamPaperPopup({courseActiveExamPaperSeq : '<c:out value="${row.courseActiveExamPaper.courseActiveExamPaperSeq}"/>',courseActiveSeq : '<c:out value="${row.courseActiveExamPaper.courseActiveSeq}"/>',activeElementSeq : '<c:out value="${row.courseActiveExamPaper.activeElementSeq}"/>',targetNumber : 'nonDegreeExamPaper_<c:out value="${row.courseActiveExamPaper.courseActiveExamPaperSeq}"/>',resultYn : 'Y',elementType : 'nonDegreeExam',onOffTypeCd : 'ON'})"><span class="result"><spring:message code="필드:시험:결과보기"/></span></a>
					</c:when>
					<c:otherwise>

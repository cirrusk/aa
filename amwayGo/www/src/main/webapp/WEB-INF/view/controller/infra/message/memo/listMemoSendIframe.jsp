...
doInitializeLocal = function() {
	...
	forSearch.config.url    = "<c:url value="/usr/memo/send/list/iframe.do"/>";
	...
	forListdata.config.url    = "<c:url value="/usr/memo/send/list/iframe.do"/>";
	...
	forDetail.config.url    = "<c:url value="/usr/memo/send/detail/iframe.do"/>";

	forDeletelist = $.action("submit", {formId : "FormData"}); // validator를 사용하는 action은 formId를 생성시 setting한다
	forDeletelist.config.url             = "<c:url value="/usr/memo/send/deletelist.do"/>";
	...
	forReceiveGroupDetail.config.url = "<c:url value="/usr/memo/send/groupdetail/popup.do"/>";
	forReceiveGroupDetail.config.options.width  = 800;
	forReceiveGroupDetail.config.options.height = 600;
				...
				<c:choose>
					...
					<c:otherwise>
						<a href="javascript:void(0)" onclick="doGroupMessageSendDetail({
															 	'messageSendSeq' : '${row.messageSend.messageSendSeq}',
																'receiveMemberNames' : '${row.receiveMember.memberName} <spring:message code="글:쪽지:외" />',

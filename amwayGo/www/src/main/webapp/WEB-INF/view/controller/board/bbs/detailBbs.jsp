...
doInitializeLocal = function() {
	...
	forListdata.config.url    = "<c:url value="/usr/system/bbs/${boardType}/list.do"/>";

	forEdit = $.action();
	forEdit.config.formId = "FormDetail";
	forEdit.config.url    = "<c:url value="/usr/system/bbs/${boardType}/edit.do"/>";
	
	forCreateReply = $.action();
	forCreateReply.config.formId = "FormCreate";
	forCreateReply.config.url    = "<c:url value="/usr/system/bbs/${boardType}/create.do"/>";
	...
	forListReply.config.url         = "<c:url value="/usr/system/bbs/${boardType}/reply/list/ajax.do"/>";
...
<c:import url="/WEB-INF/view/include/breadcrumb.jsp">
	<c:param name="suffix"><spring:message code="글:상세조회" /></c:param>
</c:import>
				...
				<div class="lybox-btn">	
					<div class="lybox-btn-r">
						...
						<c:if test="${(detailBoard.board.replyTypeCd eq CD_BOARD_REPLY_TYPE_1 and detailBbs.bbs.groupLevel eq 1 and detailBbs.bbs.replyCount eq 0)
							...
							<c:if test="${(boardType eq 'qna' or boardType eq 'free') and detailBbs.bbs.alwaysTopYn ne 'Y'}">
						...
						<c:if test="${detailBbs.bbs.regMemberSeq eq ssMemberSeq}">
							<c:if test="${boardType eq 'qna' or boardType eq 'free'}">

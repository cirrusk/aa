							...
							<c:otherwise>
								<c:choose>
									<c:when test="${row.courseActive.applyStartDate > row.courseActive.studyStartDate}">
										<a href="javascript:void(0)" onclick="doApplyRequest({'courseMasterSeq' : '${row.courseActive.courseMasterSeq}', 'courseActiveSeq' : '${row.courseActive.courseActiveSeq }'},'WAIT');" class="btn blue"><span class="small"><spring:message code="버튼:개설과목:수강신청" /></span></a><div class="vspace"></div>									
									</c:when>
									<c:otherwise>
										<a href="javascript:void(0)" onclick="doApplyRequest({'courseMasterSeq' : '${row.courseActive.courseMasterSeq}', 'courseActiveSeq' : '${row.courseActive.courseActiveSeq }'},'ING');" class="btn blue"><span class="small"><spring:message code="버튼:개설과목:수강신청" /></span></a><div class="vspace"></div>
									</c:otherwise>
								</c:choose>
							</c:otherwise>
						</c:choose>

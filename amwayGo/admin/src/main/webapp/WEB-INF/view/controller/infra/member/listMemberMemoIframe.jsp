...
doInitializeLocal = function() {
	...
	forListdata.config.url    = "<c:url value="/member/memo/list/Iframe.do"/>";
	...
	if (typeof index === "number") {
		if (form.elements["memberSeq"].length) {
			returnValue = {
				...
				phoneMobile : form.elements["phoneMobile"][index].value,
				email : form.elements["email"][index].value
			};
		} else {
			returnValue = {
				...
				phoneMobile : form.elements["phoneMobile"].value,
			...
			for (var i = 0; i < form.elements["checkkeys"].length; i++) {
				if (form.elements["checkkeys"][i].checked == true) {
					var values = {	
						...
						phoneMobile : form.elements["phoneMobile"][i].value,
						email : form.elements["email"][i].value
					};
	...
	<form name="FormSrch" id="FormSrch" method="post" onsubmit="return false;">
		<div class="lybox search">
			<fieldset class="align-l">
				...
				<input type="hidden" name="srchMessageType" value="${condition.srchMessageType}" />
				<span>
					<input type="text" name="srchCategoryName" value="${condition.srchCategoryName}" style="width:200px;" onkeyup="UT.callFunctionByEnter(event, doSearch);" />
					...
					<c:choose>
						<c:when test="${param['select'] eq 'single'}"><%-- 싱글 선택 --%>
							<td>
					        	...
					        	<input type="hidden" name="phoneMobile" value="<c:out value="${row.member.phoneMobile}" />">
					        	<input type="hidden" name="email" value="<c:out value="${row.member.email}" />">

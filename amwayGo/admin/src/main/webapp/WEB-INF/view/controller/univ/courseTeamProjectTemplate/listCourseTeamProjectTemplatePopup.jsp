...
doInitializeLocal = function() {
	...
	forSearch.config.url    = "<c:url value="/univ/course/teamproject/template/list/popup.do"/>";

	forListdata = $.action();
	forListdata.config.formId = "FormList";
	forListdata.config.url    = "<c:url value="/univ/course/teamproject/template/list/popup.do"/>";
	
	forDetail = $.action();
	forDetail.config.formId = "FormDetail";
	forDetail.config.url    = "<c:url value="/univ/course/teamproject/template/detail/popup.do"/>";
...
<body>
	...
	<c:import url="srchCourseTeamProjectTemplatePopup.jsp"/>
	...
	<thead>
		<tr>
			...
			<th><span class="sort" sortid="1"><spring:message code="필드:팀프로젝트:팀프로젝트제목" /></span></th>
	...
	<c:forEach var="row" items="${paginate.itemList}" varStatus="i">
		<tr>
	        <td>
	        	...
	        	<input type="hidden" name="templateSeq" value="<c:out value="${row.teamProjectTemplate.templateSeq}" />">
	        	<input type="hidden" name="templateTitle" value="<c:out value="${row.teamProjectTemplate.templateTitle}" />">
	        	<input type="hidden" name="templateDescription" value="<c:out value="${row.teamProjectTemplate.templateDescription}" />">
	        </td>
			<td class="align-l"><a href="javascript:doDetail({'templateSeq' : '<c:out value="${row.teamProjectTemplate.templateSeq}" />'});"><c:out value="${row.teamProjectTemplate.templateTitle}" /></a></td>
	        <td>
	        	<aof:code type="print" codeGroup="OPEN_YN" removeCodePrefix="true" selected="${row.teamProjectTemplate.openYn}"></aof:code>
	        </td>
	        <td>
	        	<c:out value="${row.member.memberName}" />
	        </td>
			<td><aof:date datetime="${row.teamProjectTemplate.regDtime}"/></td>
			<td>

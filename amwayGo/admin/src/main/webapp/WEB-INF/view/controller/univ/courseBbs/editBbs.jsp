...
doInitializeLocal = function() {
	...
	forDelete.config.url             = "<c:url value="/univ/course/bbs/delete.do"/>";
				...
				<tbody>
					...
					<c:if test="${detailBoard.board.attachCount gt 0 or !empty detailBbs.bbs.attachList}">
						<tr>
							...
							<td>
								<aof:code type="radio" name="downloadYn" codeGroup="YESNO" selected="${detailBbs.bbs.downloadYn}" removeCodePrefix="true"/>
					            </span>
							</td>
						</tr>
						<tr>
					...
					<%--<c:choose>
						<c:when test="${aoffn:contains(exceptEvaluateBoardType, boardType, ',') eq false}"> 
							<tr>
								<th><spring:message code="필드:게시판:평가대상"/></th>
								<td><aof:code type="radio" name="evaluateYn" codeGroup="YESNO" selected="${detailBbs.bbs.evaluateYn}" removeCodePrefix="true"/></td>
							</tr>
						</c:when>
						<c:otherwise>
							<input type="hidden" name="evaluateYn" value="N"/>
					...
					<tr>
						...
						<td><aof:code type="radio" name="copyYn" codeGroup="YESNO" selected="${detailBbs.bbs.copyYn}" removeCodePrefix="true"/></td>
					</tr>
					 --%>
					<c:choose>
						<c:when test="${boardType eq 'notice'}"> <%-- 공지사항 --%>
							...
							<tr>
								...
								<td>
									<aof:code type="print" name="pushYn" codeGroup="YESNO" selected="${detailBbs.bbs.pushYn}" removeCodePrefix="true"/>

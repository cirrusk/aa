...
doInitializeLocal = function() {
	...
	forEdit.config.url    = "<c:url value="/univ/course/active/exampaper/edit.do"/>";
};
/**
 * 목록보기
 */
...
<div class="vspace"></div>
	...
	<div class="lybox-title">
	 ...
	 <tbody>
	     ...
	     <tr>
	     	<th><spring:message code="필드:시험:응시시간"/></th>
	     	<td>
	 ...
	 <c:choose>
 		...
 		<c:otherwise>
 			<div class="lybox-btn">
		        ...
		        <div class="lybox-btn-r">
		            <c:if test="${aoffn:accessible(appCurrentMenu.rolegroupMenu, 'U') and (detail.courseActiveExamPaper.answerCount eq 0 or empty detail.courseActiveExamPaper.startDtime)}">
		                <a href="javascript:void(0)" onclick="doEdit({'courseActiveExamPaperSeq' : '<c:out value="${detail.courseActiveExamPaper.courseActiveExamPaperSeq}"/>'});" class="btn blue"><span class="mid"><spring:message code="버튼:수정"/></span></a>
		            </c:if>
		            <a href="#" onclick="doList();" class="btn blue"><span class="mid"><spring:message code="버튼:목록"/></span></a>
		        </div>

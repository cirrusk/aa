 ...
 */
 doDetailPopup = function(mapPKs) {
	if(mapPKs.type == "ING"){
		...
		// 상세화면 실행
		if(mapPKs.categoryTypeCd == CD_CATEGORY_TYPE_DEGREE){
	...
	<table id="listTable" class="tbl-list">
		...
		<thead>
			<tr>
				...
				<th><spring:message code="필드:수강:수강상태" /></th>
				<!-- 
		...
		<c:forEach var="row" items="${paginate.itemList}" varStatus="i">
			<tr>
		        ...
		        <td class="align-l"><c:out value="${row.active.courseActiveTitle }" /></td>
		        	...
		        	<c:choose>
		        		<c:when test="${row.category.categoryTypeCd eq CD_CATEGORY_TYPE_DEGREE }">
		        			<c:choose>
        						...
        						<c:otherwise>
        							<aof:code type="print" codeGroup="GRADE_LEVEL" selected="${row.apply.gradeLevelCd }" />
        						</c:otherwise>
        					</c:choose>
		        		</c:when>
		        		<c:otherwise>
		        ...
		        <td>
		        	<c:choose>
		        		<c:when test="${row.apply.applyStatusCd eq CD_APPLY_STATUS_002}">
		        			<c:choose>
				        		<c:when test="${row.active.studyStartDate <= today and row.active.studyEndDate >= today }">
									<a href="javascript:void(0)" onclick="doDetailPopup({'type' : 'ING', 'categoryTypeCd' : '<c:out value="${row.category.categoryTypeCd }" />', 'courseActiveSeq' : '<c:out value="${row.apply.courseActiveSeq }" />', 'courseApplySeq' : '<c:out value="${row.apply.courseApplySeq }"/>'})" class="btn gray"><span class="mid"><spring:message code="버튼:수강:조회"/></span></a>		        		
				        		</c:when>
				        		<c:otherwise>
				        			<a href="javascript:void(0)" onclick="doDetailPopup({'type' : 'END', 'categoryTypeCd' : '<c:out value="${row.category.categoryTypeCd }" />', 'courseActiveSeq' : '<c:out value="${row.apply.courseActiveSeq }" />', 'courseApplySeq' : '<c:out value="${row.apply.courseApplySeq }"/>'})" class="btn black"><span class="mid"><spring:message code="버튼:수강:조회"/></span></a>
				        		</c:otherwise>
				        	</c:choose>
		        		</c:when>
		        		<c:otherwise>

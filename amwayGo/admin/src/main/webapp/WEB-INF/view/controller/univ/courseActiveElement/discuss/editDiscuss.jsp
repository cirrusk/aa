...
doInitializeLocal = function() {
	...
	forTemplate = $.action();
	forTemplate.config.formId = "FormData";
	forTemplate.config.url    = "<c:url value="/univ/course/active/discuss/edit.do"/>";
			
	forUpdate = $.action("submit", {formId : "FormData"}); // validator를 사용하는 action은 formId를 생성시 setting한다
	forUpdate.config.url             = "<c:url value="/univ/course/active/discuss/update.do"/>";
...
<form name="FormBrowseTemplate" id="FormBrowseTemplate" method="post" onsubmit="return false;">    
	<input type="hidden" name="srchCourseActiveSeq" value="<c:out value="${detail.discuss.courseActiveSeq}"/>"/>    
	<input type="hidden" name="srchCourseMasterSeq" value="<c:out value="${param['courseMasterSeq']}"/>"/>
    <input type="hidden" name="callback" value="doTemplateInsert"/>
</form>

<form id="FormData" name="FormData" method="post" onsubmit="return false;">
    <input type="hidden" name="courseMasterSeq" value="<c:out value="${param['courseMasterSeq']}"/>"/>
    ...
    <input type="hidden" name="profMemberSeq" value="<c:out value="${detail.discuss.profMemberSeq}"/>"/>
    ...
    <tbody>
        <tr>
            ...
            <td>
                <a href="javascript:void(0)" onclick="doTemplatePopup();" class="pop-call-btn" title="<spring:message code="글:과제:템플릿팝업호출"/>" >&nbsp;</a>
                <c:choose>
                	<c:when test="${!empty discussTemplate.discussTemplate.templateTitle}">
                		<input type="text" name="discussTitle" value="${discussTemplate.discussTemplate.templateTitle}" style="width:350px;">
                	</c:when>
                	<c:otherwise>
    	            	<input type="text" name="discussTitle" value="${detail.discuss.discussTitle}" style="width:350px;">
	                </c:otherwise>
                </c:choose>
            </td>
        </tr>
        <tr>
            ...
            <td>
				...
				<c:choose>
					<c:when test="${!empty discussTemplate.discussTemplate.templateTitle}">
						<textarea name="description" id="description" value="${detail.discuss.description}" style="width:99%; height:300px"><c:out value="${discussTemplate.discussTemplate.templateDescription}" /></textarea>
					</c:when>
					<c:otherwise>
						<textarea name="description" id="description" value="${detail.discuss.description}" style="width:99%; height:300px"><c:out value="${detail.discuss.description}" /></textarea>
					</c:otherwise>
				</c:choose>
            </td>
        </tr>
        <tr>
			...
			<td>
	        	<c:choose>
	        		<%-- 템플릿이 적용된 수정페이지의 경우 --%>
	        		<c:when test="${!empty discussTemplate.discussTemplate.templateSeq}">
	        			<input type="hidden" name="attachUploadInfo"/>
		        		<%--  템플릿 적용전 삭제대상 파일셋팅 --%>
						<c:if test="${!empty detail.discuss.attachList}">
        ...
        <tr>
			...
			<td>
				<c:if test="${param['shortcutCourseTypeCd'] ne CD_COURSE_TYPE_ALWAYS}">
					...
					<input type="text" name="startDate" id="startDate" value="<aof:date datetime="${detail.discuss.startDtime}"/>" class="datepicker" style="width:80px;text-align:center;" readonly="readonly">
					<select name="startTime" style="width:90px;">
						<c:set var="startTime"><aof:date datetime="${detail.discuss.startDtime}" pattern="HHmm"/></c:set>
						<aof:code type="option" codeGroup="TIME" selected="${startTime}" removeCodePrefix="true"/>
					</select>
					~
					<input type="text" name="endDate" id="endDate" value="<aof:date datetime="${detail.discuss.endDtime}"/>" class="datepicker" style="width:80px;text-align:center;" readonly="readonly">
					<select name="endTime" style="width:90px;">
						<c:set var="endTime"><aof:date datetime="${detail.discuss.endDtime}" pattern="HHmm"/></c:set>
						<aof:code type="option" codeGroup="TIME" selected="${endTime}" removeCodePrefix="true"/>
					</select>
				</c:if>
				<c:if test="${param['shortcutCourseTypeCd'] eq CD_COURSE_TYPE_ALWAYS}">
					<spring:message code="글:수강시작"/>
	                <input type="text" name="startDay" id="startDay" class="align-r" style="width: 50px;" value="${detail.discuss.startDay}">&nbsp;
	                <spring:message code="글:일부터"/>
	                ~
	                <input type="text" name="endDay" id="endDay" class="align-r" style="width: 50px;" value="${detail.discuss.endDay}">&nbsp;
    ...
    <div class="lybox-btn">
        <div class="lybox-btn-r">
            <c:if test="${aoffn:accessible(appCurrentMenu.rolegroupMenu, 'U')}">
                <a href="javascript:void(0)" onclick="doUpdate();" class="btn blue"><span class="mid"><spring:message code="버튼:수정"/></span></a>
            </c:if>

...
doInitializeLocal = function() {
	...
	forInsert.config.url             = "<c:url value="/system/bbs/insert.do"/>";
			...
			<c:otherwise>
					...
					<tbody>
						...
						<c:if test="${boardType eq 'staff'}"> <%-- 교직원게시판 --%>
							<tr>
								...
								<td>
									<aof:code type="print" codeGroup="BBS_TARGET_ROLEGROUP" selected="${detailParentBbs.bbs.targetRolegroup}"/>
								</td>
							</tr>
						</c:if>
						<tr>
				...
				<tbody>
						...
						<td>
							...
							<c:choose>
								<c:when test="${detailBoard.board.secretYn eq 'Y'}">
									<input type="checkbox" name="secretYn" value="Y" <c:if test="${param['parentSecretYn'] eq 'Y'}">checked="checked"</c:if> />
					...
					<c:if test="${boardType eq 'staff'}"> <%-- 교직원게시판 --%>
						<c:choose>
							<c:when test="${replyYn eq 'Y'}">
								<input type="hidden" name="targetRolegroup" value="<c:out value="${detailParentBbs.bbs.targetRolegroup}"/>"/>
							</c:when>
							<c:otherwise>
								<tr>
									<th><spring:message code="필드:게시판:대상자"/><span class="star">*</span></th>
									<td>
										<aof:code type="checkbox" name="targetRolegroup" codeGroup="BBS_TARGET_ROLEGROUP"/>
									</td>
								</tr>
							</c:otherwise>
						</c:choose>

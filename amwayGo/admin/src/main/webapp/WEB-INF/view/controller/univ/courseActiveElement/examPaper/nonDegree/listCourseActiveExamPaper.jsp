...
doInitializeLocal = function() {
    ...
    forDetail.config.url    = "<c:url value="/univ/course/active/exampaper/detail.do"/>";
    
    forCreate = $.action();
    forCreate.config.formId = "FormCreate";
    forCreate.config.url    = "<c:url value="/univ/course/active/exampaper/create.do"/>";
    
    forUpdateRate = $.action("submit", {formId : "FormData"}); // validator를 사용하는 action은 formId를 생성시 setting한다
    forUpdateRate.config.url             = "<c:url value="/univ/course/active/exampaper/rate/update.do"/>";
    ...
    forDeletelist.config.url             = "<c:url value="/univ/course/active/exampaper/deletelist.do"/>";
...
doRateSum = function(obj){
    ...
    if(rateSum > 100){
    	$.alert({message : "<spring:message code='글:시험:평가비율총합은100%이어야합니다'/>"});
    }
};
    ...
    <div class="lybox-title">
        ...
        <div class="right">
            <c:if test="${aoffn:accessible(menuActiveDetail.rolegroupMenu, 'R')}">
                <a href="javascript:void(0);" class="btn gray" onclick="FN.doGoMenu('<c:url value="${menuActiveDetail.menu.url}"/>','<c:out value="${aoffn:encrypt(menuActiveDetail.menu.menuId)}"/>','<c:out value="${menuActiveDetail.menu.dependent}"/>','<c:out value="${menuActiveDetail.menu.urlTarget}"/>');" ><span class="small"><spring:message code="버튼:시험:시험결과" /></span></a>
            </c:if>
        </div>
    </div>
	    ...
	    <thead>
	        <tr>
	            ...
	            <c:choose>
	            	...
	            	<c:otherwise>
			            <th><spring:message code="필드:시험:시작일" />|<spring:message code="필드:시험:종료일" /></th>
	            	</c:otherwise>
	            </c:choose>

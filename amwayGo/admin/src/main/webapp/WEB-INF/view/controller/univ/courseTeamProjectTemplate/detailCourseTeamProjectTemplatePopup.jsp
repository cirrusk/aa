...
<body>
	...
	<input type="hidden" name="templateSeq" value="<c:out value="${detail.teamProjectTemplate.templateSeq}"/>">
	<input type="hidden" name="templateTitle" value="<c:out value="${detail.teamProjectTemplate.templateTitle}"/>">
	<input type="hidden" name="templateDescription" value="<c:out value="${detail.teamProjectTemplate.templateDescription}"/>">
	...
	<c:if test="${!empty detail.teamProjectTemplate.attachList}">
		...
		<tr>
			...
			<td><aof:code type="print" codeGroup="OPEN_YN" removeCodePrefix="true" selected="${detail.teamProjectTemplate.openYn}"></aof:code></td>
		</tr>
		<tr>
		...
		<div class="lybox-btn-l">
			<c:if test="${aoffn:accessible(appCurrentMenu.rolegroupMenu, 'D')}">
				<c:choose>
					...
					<c:otherwise>
						<c:if test="${(condition.srchProfMemberSeq eq detail.teamProjectTemplate.profMemberSeq) or (memberSeq eq detail.teamProjectTemplate.regMemberSeq)}">
							<a href="javascript:void(0)" onclick="doDelete();" class="btn blue"><span class="mid"><spring:message code="버튼:삭제"/></span></a>
						</c:if>
					</c:otherwise>
				</c:choose>

...
doInitializeLocal = function() {
	...
	forEdit.config.url    = "<c:url value="/univ/course/discuss/template/edit.do"/>";
...
<body>
	...
	<div class="modify">
		...
		<span><c:out value="${detail.discussTemplate.updMemberName}"/></span>
		&nbsp;
		<strong><spring:message code="필드:수정일시"/></strong>
		<span class="date"><aof:date datetime="${detail.discussTemplate.updDtime}" pattern="${aoffn:config('format.datetime')}"/></span>
	</div>
	...
	<c:if test="${!empty detail.discussTemplate.attachList}">
		...
		<tr>
			<th><spring:message code="필드:토론:토론내용"/></th>
			<td><aof:text type="whiteTag" value="${detail.discussTemplate.templateDescription}"/></td>
		</tr>
		<tr>
			<th><spring:message code="필드:템플릿:사용여부"/></th>
			<td><aof:code type="print" codeGroup="USEYN" removeCodePrefix="true" selected="${detail.discussTemplate.useYn}"></aof:code></td>
		</tr>
	</tbody>
	</table>
	</form>
		...
		<div class="lybox-btn-r">
			<c:if test="${aoffn:accessible(appCurrentMenu.rolegroupMenu, 'U')}">
				<c:if test="${currentRoleCfString eq 'ADM'}"><!-- 관리자일시 -->
					<a href="#" onclick="doEdit({'templateSeq' : '<c:out value="${detail.discussTemplate.templateSeq}"/>'});"
						class="btn blue"><span class="mid"><spring:message code="버튼:수정"/></span></a>
				</c:if>
				<c:if test="${currentRoleCfString eq 'PROF'}"><!-- 교강사일시 -->
					<c:if test="${detail.discussTemplate.profMemberSeq eq memberSeq}">
						<a href="#" onclick="doEdit({'templateSeq' : '<c:out value="${detail.discussTemplate.templateSeq}"/>'});"
							class="btn blue"><span class="mid"><spring:message code="버튼:수정"/></span></a>
					</c:if>
				</c:if>
				<c:if test="${currentRoleCfString eq 'ASSIST' or currentRoleCfString eq 'TUTOR'}"><!-- 선임, 강사일시 -->
					<c:if test="${(delUpYn eq 'Y') or (detail.discussTemplate.regMemberSeq eq memberSeq)}">
						<a href="#" onclick="doEdit({'templateSeq' : '<c:out value="${detail.discussTemplate.templateSeq}"/>'});"
							class="btn blue"><span class="mid"><spring:message code="버튼:수정"/></span></a>
					</c:if>
				</c:if>

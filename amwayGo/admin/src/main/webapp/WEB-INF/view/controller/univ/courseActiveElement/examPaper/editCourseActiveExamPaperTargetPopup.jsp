...
doInitializeLocal = function() {
	...
	forSearch.config.url    = "<c:url value="/univ/course/active/exampaper/target/popup.do"/>";
	
	forInsertlist = $.action("submit", {formId : "FormData"}); // validator를 사용하는 action은 formId를 생성시 setting한다
	forInsertlist.config.url             = "<c:url value="/univ/course/active/exampaper/target/insertlist.do"/>";
	forInsertlist.config.target          = "hiddenframe";
	forInsertlist.config.message.confirm = "<spring:message code="글:시험:대상자를지정하시겠습니까"/>"; 
	...
	forInsertlist.validator.set({
		title : "<spring:message code="글:시험:지정할대상자"/>",
		name : "nonTargetApplyCheckkeys",
		data : ["!null"]
	});
	
	forDeletelist = $.action("submit", {formId : "FormData"}); // validator를 사용하는 action은 formId를 생성시 setting한다
	forDeletelist.config.url             = "<c:url value="/univ/course/active/exampaper/target/deletelist.do"/>";
	forDeletelist.config.target          = "hiddenframe";
	forDeletelist.config.message.confirm = "<spring:message code="글:시험:대상자를제외하시겠습니까"/>"; 
	...
	forDeletelist.validator.set({
		title : "<spring:message code="글:시험:제외할대상자"/>",
...
<form id="FormSrch" name="FormSrch" method="post" onsubmit="return false;">
	<input type="hidden" name="courseActiveExamPaperSeq" value="<c:out value='${condition.courseActiveExamPaperSeq}'/>">
	        ...
	        <tr>
	            <th>
	                <div class="lybox-title">
	                    <h4 class="section-title"><spring:message code="필드:시험:비대상자목록"/></h4>
	                </div>
	            </th>
	            <th></th>
	            <th>
	                <div class="lybox-title">
	                    <h4 class="section-title"><spring:message code="필드:시험:대상자목록"/></h4>
	                </div>
	            </th>
	        </tr>
	        <tr>
            ...
            <td class="first"><!-- first -->
                ...
                <div class="scroll-y" style="height:360px;">
                    <table class="tbl-list" id="nonTargetTable"><!-- tbl-list -->
                        <colgroup>
                        	<c:if test="${condition.editYn eq 'Y'}">
                        ...
                        <tbody id="nonTargetMemberArea">
                            ...
                            <tr>
	                            <c:if test="${condition.editYn eq 'Y'}">
	                                <td>
	                                    <input type="checkbox" name="nonTargetApplyCheckkeys" value="<c:out value="${row.courseActiveExamPaperTarget.courseApplySeq}"/>" onclick="FN.onClickCheckbox(this)">
	                                </td>
	                            </c:if>
                                <td>
                                ...
                                <td>
                                    ...
                                    <spring:message code="필드:시험:학년" />
                                </td>
                                <td>
                                    <c:out value="${row.courseActiveExamPaperTarget.takeScore}"/>
                                </td>
                            </tr>
                            </c:forEach>
                            <tr id="UnassignNoDataArea" <c:if test="${not empty listNonTarget}">style="display: none;"</c:if>>
                                ...
                                <c:if test="${condition.editYn eq 'N'}">
                    ...
                    <table class="tbl-list" id="targetTable">
                        ...
                        <tbody id="targetMemberArea">
                           <c:forEach var="row" items="${listTarget}" varStatus="i">
                            ...
                            <c:if test="${condition.editYn eq 'Y'}">
                                <td>
                                    <input type="checkbox" name="targetApplyCheckkeys" value="<c:out value="${row.courseActiveExamPaperTarget.courseApplySeq}"/>" onclick="FN.onClickCheckbox(this, 'checkButtonBottom')">
                                </td>
                            </c:if>
                                <td>
                                    <c:out value="${row.member.memberName}"/>
        ...
        <tr>
        	<td>
        		<div class="lybox-btn-l">
			        <div class="lybox-title">
			            <h4 class="section-title"><spring:message code="필드:시험:총"/> : 
			                <span class="count"><c:out value="${fn:length(listNonTarget)}"/><spring:message code="필드:시험:명"/></span>
			            </h4>
			        </div>
			    </div>
			    <c:if test="${condition.editYn eq 'Y'}">
				    <div class="lybox-btn-r">
					    <a href="javascript:void(0)" onclick="doInsertlist()" class="btn gray"><span class="small"><spring:message code="버튼:시험:지정"/> ></span></a>
					</div>
				</c:if>
        	</td>
        	<td>
        		<c:if test="${condition.editYn eq 'Y'}">
	        		<div class="lybox-btn-l">
				        <a href="javascript:void(0)" onclick="doDeletelist()" class="btn gray"><span class="small">< <spring:message code="버튼:시험:제외"/></span></a>
				    </div>
			    </c:if>
        		<div class="lybox-btn-r">
			        <div class="lybox-title">
			            <h4 class="section-title"><spring:message code="필드:시험:총"/> : 
			                <span class="count"><c:out value="${fn:length(listTarget)}"/><spring:message code="필드:시험:명"/></span>
			            </h4>
			        </div>
			    </div>
        	</td>

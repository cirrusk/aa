...
doInitializeLocal = function() {
	...
	forInsert.config.url             = "<c:url value="/schedule/${referenceType}/insert.do"/>";
...
doSetDatetime = function() {
	...
	// 요일세팅
	var days = doGetDayofweek(pickerStart, pickerEnd);
	var $week = $form.find(":input[name='repeatWeek']");
	$week.attr("checked", false);
	for (var index in days) {
		$week.filter("[value='" + days[index] + "']").attr("checked", true);
	}
	doSetRepeatType();
};
/**
...
doSetRepeatType = function() {
	...
	$repeatType.attr("checked", false).hide().next("label").hide();
	...
	<tbody>
		<tr>
			...
			<td colspan="3">
				...
				<input type="text" name="pickerStart" id="pickerStart" value="<aof:date datetime="${param['startDtime']}"/>" class="datepicker" style="width:80px;text-align:center;" readonly="readonly" onchange="doSetDatetime()">
				<select name="timeStart" onchange="doSetDatetime()" style="width:90px;">
					<c:set var="timeStart"><aof:date datetime="${param['startDtime']}" pattern="HHmm"/></c:set>
					<aof:code type="option" codeGroup="TIME" selected="${timeStart}" removeCodePrefix="true"/>
				</select>
				-
				<input type="text" name="pickerEnd" id="pickerEnd" value="<aof:date datetime="${param['endDtime']}"/>" class="datepicker" style="width:80px;text-align:center;" readonly="readonly" onchange="doSetDatetime()">
				<select name="timeEnd" onchange="doSetDatetime()" style="width:90px;">
					<c:set var="timeEnd"><aof:date datetime="${param['endDtime']}" pattern="HHmm"/></c:set>
		...
		<tr>
			...
			<td colspan="3"><input type="text" name="scheduleTitle" style="width:90%;"></td>
		</tr>
		<tr>
			...
			<td colspan="3">
				<c:choose>
					<c:when test="${referenceType eq 'system'}">
						<aof:code type="radio" codeGroup="SCHEDULE_TYPE" name="scheduleTypeCd" defaultSelected="${CD_SCHEDULE_TYPE_01}" except="${CD_SCHEDULE_TYPE_05}"/> <%-- 개인일정 제외 --%>
					</c:when>
					<c:otherwise>
		...
		<tr>
			...
			<td><aof:code type="radio" codeGroup="YESNO" name="repeatYn" defaultSelected="N" onclick="doChangeRepeatYn()" removeCodePrefix="true"/></td>
		...
		<tr class="repeat">
			...
			<td colspan="3" style="position:relative;">
				<aof:code type="radio" codeGroup="SCHEDULE_REPEAT_TYPE" name="repeatTypeCd" onclick="doChangeRepeatType()"/>
				...
				<span style="position:absolute;right:0px;display:none;" id="week">
					<aof:code type="checkbox" codeGroup="${week}" name="repeatWeek" onclick="doChangeRepeatType()"/>
				</span>
			</td>
		</tr>
		<tr class="repeat">
			...
			<td colspan="3">
				<input type="text" name="repeatEndDate" id="repeatEndDate" class="datepicker" style="width:80px;text-align:center;" readonly="readonly">
				<input type="checkbox" name="unlimitedYn" value="Y" onclick="doChangeRepeatEndDate(this)" checked>

...
doListRefresh = function() {
	...
	if (parseInt(level, 10) == 1) {
		...
		action.config.url = "<c:url value="/category/{0}/list.do"/>".format({0:"<c:out value="${category.categoryTypeCd}"/>".replace(CD_CATEGORY_TYPE_ADDSEP, "").toLowerCase()});
		action.run();
	} else {
...
<body>
	...
	<c:if test="${category.categoryTypeCd eq CD_CATEGORY_TYPE_CONTENTS}">

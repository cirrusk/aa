...
doInitializeLocal = function() {
    ...
    forListdata.config.url    = "<c:url value="/univ/course/active/quiz/result/list.do"/>";

    forDetail = $.action();
    forDetail.config.formId = "FormSubList";
    forDetail.config.url    = "<c:url value="/univ/course/active/quiz/result/detail.do"/>";
    
    forQuizResultPopup = $.action("layer");
    forQuizResultPopup.config.formId         = "FormQuiz";
    forQuizResultPopup.config.url            = "<c:url value="/univ/course/active/exampaper/result/detail/popup.do"/>";
    forQuizResultPopup.config.options.width  = 900;
    forQuizResultPopup.config.options.height = 700;
};
/**
 * 목록보기
 */
 ...
 * 채점하기 팝업
 */
doQuizResultPopup = function(mapPKs) {
	// 상세화면 form을 reset한다.
    UT.getById(forQuizResultPopup.config.formId).reset();
    // 상세화면 form에 키값을 셋팅한다.
    UT.copyValueMapToForm(mapPKs, forQuizResultPopup.config.formId);
    // 팝업 실행
	forQuizResultPopup.run();
}; 
</script>
</head>
...
<c:set var="srchKey">memberName=<spring:message code="필드:시험:이름"/>,memberId=<spring:message code="필드:시험:아이디"/></c:set>
		...
		<table id="listTable" class="tbl-list mt20">
			...
			<thead>
				<tr>
					...
					<th><spring:message code="필드:퀴즈:이름"/></th>
					<th><spring:message code="필드:퀴즈:아이디"/></th>
					<th><spring:message code="필드:퀴즈:학과"/></th>
					<th>IP</th>
					<th><spring:message code="필드:퀴즈:시작시간"/></th>
					<th><spring:message code="필드:퀴즈:종료시간"/></th>
					<th><spring:message code="필드:퀴즈:객관식점수"/>(<spring:message code="글:퀴즈:점"/>)</th>
					<th><spring:message code="필드:퀴즈:단답형점수"/>(<spring:message code="글:퀴즈:점"/>)</th>
					<th><spring:message code="필드:퀴즈:서술형점수"/>(<spring:message code="글:퀴즈:점"/>)</th>
					<th><spring:message code="필드:퀴즈:획득점수"/>(<spring:message code="글:퀴즈:점"/>)</th>
					<th><spring:message code="필드:퀴즈:채점일"/></th>
					<th><spring:message code="필드:퀴즈:상세"/></th>
				</tr>
			</thead>
			<tbody>
				...
				<tr>
					...
					<c:choose>
						<c:when test="${row.courseApplyElement.completeYn eq 'Y'}">
							...
							<td>
								<a href="#" onclick="doQuizResultPopup({'courseApplySeq' : '<c:out value="${row.courseApplyElement.courseApplySeq}"/>', 'activeElementSeq' : '<c:out value="${row.courseApplyElement.activeElementSeq}"/>',scoreYn : '<c:out value="${empty row.courseActiveExamPaperTarget.scoreYn ? 'W' : row.courseActiveExamPaperTarget.scoreYn}"/>'});" class="btn black"><span class="small"><spring:message code="버튼:시험:보기"/></span></a>
							</td>
						</c:when>
						<c:otherwise>
							<td>
	...
	<form name="FormQuiz" id="FormQuiz" method="post" onsubmit="return false;">
		<input type="hidden" name="courseActiveExamPaperSeq" value="<c:out value="${detail.courseActiveExamPaper.courseActiveExamPaperSeq}"/>" />
		<input type="hidden" name="courseActiveSeq" value="<c:out value="${detail.courseActiveExamPaper.courseActiveSeq}"/>" />

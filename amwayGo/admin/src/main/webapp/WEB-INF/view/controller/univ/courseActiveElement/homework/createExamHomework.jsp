...
doInitializeLocal = function() {
	...
	<c:choose>
		<c:when test="${homework.referenceType eq 'EXAM'}"><%-- 일반과정이 시험일 경우 --%>
	forScoreTarget.config.url             = "<c:url value="/univ/course/active/{examType}/exampaper/target/score/count.do"/>";
		</c:when>
		<c:when test="${homework.referenceType eq 'HOMEWORK'}"><%-- 일반과정이 대체과제일 경우 --%>
	...
	forTemplate.config.url    = "<c:url value="/univ/course/active/${examType}/homework/create.do"/>";
...
setValidate = function() {
	...
	forInsert.validator.set({
        title : "<spring:message code="필드:과제:2차과제시작기간"/>",
        name : "start2Date",
        data : ["!null"],
        when : function() {
			var form = UT.getById(forInsert.config.formId);
...
<div id="tabContainer">
	...
	<div class="lybox-title"><!-- lybox-title -->
	    <h4 class="section-title">
	    	<c:if test="${homework.basicSupplementCd ne CD_BASIC_SUPPLEMENT_SUPPLEMENT}">
	    		<c:choose>
					<c:when test="${examType eq 'middle'}">
						<spring:message code="필드:과제:중간고사대체과제등록" />
					</c:when>
					<c:otherwise>
						<spring:message code="필드:과제:기말고사대체과제등록" />
					</c:otherwise>
				</c:choose>
	    	</c:if>
	    	<c:if test="${homework.basicSupplementCd eq CD_BASIC_SUPPLEMENT_SUPPLEMENT}">
	    		<c:choose>
					<c:when test="${examType eq 'middle'}">
						<spring:message code="필드:과제:중간고사미응시자보충과제등록" />
					</c:when>
					<c:otherwise>
						<spring:message code="필드:과제:기말고사미응시자보충과제등록" />
					</c:otherwise>
				</c:choose>
	    	</c:if>
	    </h4>
	...
	<form id="FormInsert" name="FormInsert" method="post" onsubmit="return false;">
	    ...
	    <input type="hidden" name="referenceType"   value="<c:out value="${homework.referenceType}"/>"/>
	    ...
	    <c:if test="${homework.basicSupplementCd ne CD_BASIC_SUPPLEMENT_SUPPLEMENT}"><!-- 일반 비율 100 -->
	    ...
	    <tbody>
	     ...
	     <c:if test="${homework.basicSupplementCd eq CD_BASIC_SUPPLEMENT_BASIC}">
	     	<tr>

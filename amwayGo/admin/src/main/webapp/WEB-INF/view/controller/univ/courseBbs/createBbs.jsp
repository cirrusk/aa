...
doInitializeLocal = function() {
	...
	forInsert.config.url             = "<c:url value="/univ/course/bbs/insert.do"/>";
...
doInsert = function() { 
	// editor 값 복사
	UI.editor.copyValue();
	//var chatYn = $(':radio[name="chattingYn"]:checked').val();
	forInsert.run();
};
/**
 * 목록보기
			...
			<c:otherwise>
				...
				<tbody>
					...
					<tr>
						<th><spring:message code="필드:게시판:제목"/></th>
						<td>
							<!-- <input type="text" name="bbsTitle" style="width:550px; height:60px;"> -->
							<textarea type="text" name="bbsTitle" style="width:550px; height:70px;" rows="3"></textarea>
							<c:choose>
								<c:when test="${replyYn eq 'Y'}">
					...
					<c:if test="${detailBoard.board.attachCount gt 0}">
						...
						<tr>
							<th>바로가기</th>
							<td>
								<%-- <aof:code type="radio" name="chatYn" codeGroup="YESNO" selected="Y" removeCodePrefix="true"/> --%>
								<input type="checkbox" id="chatYn" name="evaluateYns" value="C" onclick="onoffDiv(this.value);"/>채팅&nbsp;
								<input type="checkbox" id="chatYn" name="evaluateYns" value="H" onclick="onoffDiv(this.value);"/>과제&nbsp;
								<input type="checkbox" id="chatYn" name="evaluateYns" value="R" onclick="onoffDiv(this.value);"/>실시간&nbsp;
								<input type="checkbox" id="chatYn" name="evaluateYns" value="T" onclick="onoffDiv(this.value);"/>팀활동&nbsp;
								<input type="checkbox" id="chatYn" name="evaluateYns" value="S" onclick="onoffDiv(this.value);"/>설문&nbsp;
							</td>
						</tr>
						<tr>
							<th>외부링크</th>
							<td>
								<input type="text" id="linkName" name="linkName" placeholder="링크명" /> 
								<input type="text" id="linkText" name="linkText" value="http://" placeholder="링크URL" style="width:300px;"/>
							</td>
						</tr>						
						<tr>
							<th><spring:message code="필드:게시판:첨부파일"/></th>
					...
					<input type="hidden" name="copyYn" value="N"/>
					...
					<tr>
					 ...
					 <tr>
						...
						<td>
							<aof:code type="radio" name="pushYn" codeGroup="YESNO" selected="Y" removeCodePrefix="true"/>
							&nbsp;<span id="warning" style="color: red;">
				                	※ <spring:message code="필드:게시판:푸시발송은신규등록시에만발송됩니다" />
				            </span>
						</td>
					</tr>
				</tbody>

...
doInitializeLocal = function() {
	...
	forFileDownload = $.action("submit");
	forFileDownload.config.formId = "FormOutputDownload";
	forFileDownload.config.url = "<c:url value="/cdms/output/file/download.do"/>";
	forFileDownload.config.message.waiting = "<spring:message code="글:처리중입니다"/>";
...
doOutputFile = function(element) {
	...
	var $form = jQuery("#" + forFileDownload.config.formId);
	$form.find(":input[name='rootDonwloadName']").val($element.attr("zip"));
 ...
 * 파일/폴더 다운로드
 */
doDownloadOutputFile = function() {
	var $form = jQuery("#" + forFileDownload.config.formId);
	var del = $form.find(":input[name='downOption']").filter(":checked").val();
...
<body>
	...
	<tr>
		...
		<td>
			<div class="lybox-nobg scroll-y" style="height:460px;">
				<div id="file-home">
			...
			<div class="lybox-btn lybox-nobg mt10" style="padding:0;">
				<div class="lybox-btn-l">
					<form name="FormOutputDownload" id="FormOutputDownload" method="post" onsubmit="return false;">
						<input type="hidden" name="rootDonwloadName"/>
						<input type="hidden" name="filepath"/>
						<div class="options"><input type="radio" name="downOption" value="select" checked="checked"> <label><spring:message code="글:CDMS:선택된파일또는폴더다운로드"/></label></div>
						<div class="options"><input type="radio" name="downOption" value="root"> <label><spring:message code="글:CDMS:루트폴더다운로드"/>(<spring:message code="글:CDMS:전체다운로드"/>)</label></div>
					</form>
				</div>
				<div class="lybox-btn-r">
					<a href="javascript:void(0)" onclick="doDownloadOutputFile()" class="btn blue"><span class="mid"><spring:message code="버튼:CDMS:다운로드" /></span></a>
				</div>
			</div>
		</td>
	</tr>

...
doInitializeLocal = function() {
	...
	forEdit.config.url    = "<c:url value="/cdms/bbs/${boardType}/edit.do"/>";
	
	forCreateReply = $.action();
	forCreateReply.config.formId = "FormCreate";
	forCreateReply.config.url    = "<c:url value="/cdms/bbs/${boardType}/create.do"/>";
	...
	forListReply.config.url         = "<c:url value="/cdms/bbs/${boardType}/reply/list/ajax.do"/>";
		...
		<c:choose>
			...
			<c:otherwise>
				...
				<tbody>
					...
					<c:if test="${!empty detailBbs.project.projectSeq}">
						<tr>
							<th><spring:message code="필드:CDMS:과정명"/></th>
							<td><c:out value="${detailBbs.project.projectName}"/></td>
						</tr>
					</c:if>
					<c:if test="${!empty detailBbs.section.sectionIndex}">
						<tr>
							<th><spring:message code="필드:CDMS:개발단계"/></th>
							<td><c:out value="${detailBbs.section.sectionName}"/></td>
						</tr>
					</c:if>
					<tr>

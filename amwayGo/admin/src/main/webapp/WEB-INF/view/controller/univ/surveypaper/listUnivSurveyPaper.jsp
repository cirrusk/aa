...
doInitializeLocal = function() {
	...
	forSearch.config.url    = "<c:url value="/univ/surveypaper/list.do"/>";
	...
	forCreate.config.url    = "<c:url value="/univ/surveypaper/create.do"/>";
...
<body>
	...
	<thead>
		<tr>
			...
			<th><span class="sort" sortid="3"><spring:message code="필드:설문:설문지구분" /></span></th>
			<th><span class="sort" sortid="4"><spring:message code="필드:설문:운영과정" /><spring:message code="필드:설문:활용수" /></span></th>
		...
		<tr>
			...
			<td class="align-l">
				<a href="javascript:doDetail({
					'surveyPaperSeq' : '${row.univSurveyPaper.surveyPaperSeq}'
				});"><c:out value="${row.univSurveyPaper.surveyPaperTitle}" /></a>
			</td>
			<td><aof:code type="print" codeGroup="SURVEY_PAPER_TYPE" selected="${row.univSurveyPaper.surveyPaperTypeCd}"/></td>
			<td><c:out value="${row.univSurveyPaper.useCount}"/></td>
			<td><aof:code type="print" codeGroup="YESNO" ref="2" selected="${row.univSurveyPaper.useYn}" removeCodePrefix="true"/></td>
			<td><aof:date datetime="${row.univSurveyPaper.regDtime}"/></td>
		</tr>
	</c:forEach>

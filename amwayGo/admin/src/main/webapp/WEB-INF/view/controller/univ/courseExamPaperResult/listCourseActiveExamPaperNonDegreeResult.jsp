...
doInitializeLocal = function() {
	...
	forDetailExamPaper.config.url    = "<c:url value="/univ/course/active/exampaper/result/detail.do"/>";
		...
		<div class="lybox-title">
		    <div class="right">
		    	<c:if test="${aoffn:accessible(menuActiveDetail.rolegroupMenu, 'R')}">
					<a href="#" class="btn gray" onclick="FN.doGoMenu('<c:url value="/univ/course/active/exampaper/list.do"/>','<c:out value="${aoffn:encrypt(menuActiveDetail.menu.menuId)}"/>','<c:out value="${menuActiveDetail.menu.dependent}"/>','<c:out value="${menuActiveDetail.menu.urlTarget}"/>');"><span class="small"><spring:message code="버튼:설정" /></span></a>
		    	</c:if>
		    </div>
		</div>             
    </c:if>

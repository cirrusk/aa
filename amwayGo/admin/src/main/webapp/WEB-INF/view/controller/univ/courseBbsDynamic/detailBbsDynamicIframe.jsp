...
doInitializeLocal = function() {
	...
	forEdit.config.url    = "<c:url value="/univ/course/bbs/dynamic/${boardSeq}/edit/iframe.do"/>";
	
	forCreateReply = $.action();
	forCreateReply.config.formId = "FormCreate";
	forCreateReply.config.url    = "<c:url value="/univ/course/bbs/dynamic/${boardSeq}/create/iframe.do"/>";
	...
	forListReply.config.url         = "<c:url value="/univ/course/bbs/dynamic/${boardSeq}/reply/list/ajax.do"/>";
				...
				<tbody>
					...
					<c:if test="${aoffn:contains(exceptEvaluateBoardType, boardType, ',') eq false}"> <%-- 평가대상 --%>
						<tr>
							...
							<td><aof:code type="print" name="evaluateYn" codeGroup="YESNO" selected="${detailBbs.bbs.evaluateYn}" removeCodePrefix="true"/></td>
						</tr>
					</c:if>
					<tr>
						<th><spring:message code="필드:게시판:복사대상"/></th>
						<td><aof:code type="print" codeGroup="YESNO" selected="${detailBbs.bbs.copyYn}" removeCodePrefix="true"/></td>
					</tr>
				</tbody>
				</table>

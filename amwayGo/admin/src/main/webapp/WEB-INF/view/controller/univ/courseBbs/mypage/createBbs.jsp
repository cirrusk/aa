...
doInitializeLocal = function() {
	...
	forInsert.config.url             = "<c:url value="/mypage/univ/course/group/bbs/insert.do"/>";
	...
	<c:otherwise>
		...
		<c:choose>
			...
			<c:otherwise>
				...
				<c:if test="${replyYn eq 'Y'}">
					<c:forEach var="row" items="${detailCourseActiveBbs}" varStatus="i">
						<input type="hidden" name="courseActiveSeqs" value="${row.courseActive.courseActiveSeq}" />
					</c:forEach>
				</c:if>
								...
								<c:choose>
									<c:when test="${row.category.categoryTypeCd eq CD_CATEGORY_TYPE_DEGREE}">
										<c:if test="${row.courseActive.yearTerm eq systemYearTerm}">
											<input type="checkbox" name="courseActiveSeqs" value="${row.courseActive.courseActiveSeq}"/>

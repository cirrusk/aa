...
<table class="tbl-detail">
    ...
    <tbody> 
        ...
        <tr> 
			...
			<th><spring:message code="필드:성적관리:환산학점" /></th> 
			<th><spring:message code="필드:성적관리:등수" /></th> 
        </tr>
        <tr> 
			<td><c:out value="${detail.apply.addScore }" default="0" /><spring:message code="글:성적관리:점" /></td>
			<td><c:out value="${detail.apply.finalScore }"  /></td>
			<td><aof:code type="print" codeGroup="GRADE_LEVEL" selected="${detail.apply.gradeLevelCd }" /></td>
			<td><c:out value="${detail.apply.ranking }" />/<c:out value="${detail.member.totalCount }" /></td>
        </tr>
    </tbody> 
</table>
        ...
        <tr>
        	...
        	<td>
        		...
        		<c:if test="${row.evaluate.evaluateTypeCd eq CD_COURSE_ELEMENT_TYPE_ORGANIZATION}">
        			...
        			<c:if test="${not empty row.element.totalProgressMeasure}">
        		...
        		<c:if test="${row.evaluate.evaluateTypeCd eq CD_COURSE_ELEMENT_TYPE_OFFLINE }">
        			<c:out value="${row.element.offlineAttendTypeCnt }" /> / <c:out value="${row.element.offlineAttendCnt }" /><c:set var="offlineAttendCnt" value="${row.element.offlineAttendCnt }" />
        		</c:if>
        	</td> 
        	<td>
        		<c:choose>
        			...
        			<c:when test="${row.evaluate.evaluateTypeCd eq CD_COURSE_ELEMENT_TYPE_MIDEXAM }">
        				<c:set var="score" value="${row.apply.middleExamScore }" />
        			</c:when>
        			<c:when test="${row.evaluate.evaluateTypeCd eq CD_COURSE_ELEMENT_TYPE_FINALEXAM }">
        				<c:set var="score" value="${row.apply.finalExamScore }" />
        			</c:when>
        			<c:when test="${row.evaluate.evaluateTypeCd eq CD_COURSE_ELEMENT_TYPE_HOMEWORK }">
...
<c:if test="${organizationEvaluateScore > 0 }">
			...
			<tr>
				...
				<c:forEach var="rowSub" items="${row.itemResultList}" varStatus="j"><!-- 강 리스트 -->
					...
					<tr>
						...
						<td>
							<span class="profressPrint">
							    <c:if test="${not empty rowSub.learnerDatamodel.progressMeasure}">
								    <aof:number value="${rowSub.learnerDatamodel.progressMeasure * 100}" pattern="#,###.#"/>%
								</c:if>
							</span>
							<span class="profressText" style="display: none;">
							    <c:set var="progressMeasures" value="0"></c:set>
                                 <c:if test="${not empty rowSub.learnerDatamodel.progressMeasure}">
                                     <c:set var="progressMeasures" value="${rowSub.learnerDatamodel.progressMeasure * 100}"></c:set>
                                 </c:if>
                                 
								<input type="text" name="progressMeasures" value="<aof:number value="${progressMeasures * 100}" pattern="#,###.#"/>" size="5" class="align-r">%
							</span>
							<input type="hidden" name="oldProgressMeasures" value="<aof:number value="${progressMeasures * 100}" pattern="#,###.#"/>">
	...
	<table class="tbl-detail">
	    ...
	    <tbody> 
	        ...
	        <tr>
	            <td><c:if test="${not empty avgProgressMeasure}"><aof:number value="${avgProgressMeasure }" pattern="#,###.#" />%</c:if></td>
...
<c:if test="${midExamEvaluateScore > 0 }">
	<div class="vspace"></div>
	<div class="lybox-tbl"> 
		<h4 class="title"><spring:message code="필드:성적관리:중간고사" /></h4> 
	</div>
	<table class="tbl-list">
		...
		<c:forEach var="row" items="${applyElementList}" varStatus="i">
			<c:if test="${row.activeElement.referenceTypeCd eq CD_COURSE_ELEMENT_TYPE_MIDEXAM }">
			<tr>
				<td>
					<c:choose>
						<c:when test="${row.activeElement.courseWeekTypeCd eq CD_COURSE_WEEK_TYPE_MIDEXAM and row.applyElement.basicSupplementCd eq CD_BASIC_SUPPLEMENT_BASIC }">
							<aof:code type="print" codeGroup="COURSE_ELEMENT_TYPE" selected="${row.activeElement.referenceTypeCd }" />
						</c:when>
						<c:when test="${row.activeElement.courseWeekTypeCd eq CD_COURSE_WEEK_TYPE_MIDEXAM and row.applyElement.basicSupplementCd eq CD_BASIC_SUPPLEMENT_SUPPLEMENT }">
							<spring:message code="필드:성적관리:보충시험" />
						</c:when>
						<c:when test="${row.activeElement.courseWeekTypeCd eq CD_COURSE_WEEK_TYPE_MIDHOMEWORK and row.applyElement.basicSupplementCd eq CD_BASIC_SUPPLEMENT_BASIC }">
							<spring:message code="필드:성적관리:대체과제" />
						</c:when>
						<c:when test="${row.activeElement.courseWeekTypeCd eq CD_COURSE_WEEK_TYPE_MIDHOMEWORK and row.applyElement.basicSupplementCd eq CD_BASIC_SUPPLEMENT_SUPPLEMENT }">
							<spring:message code="필드:성적관리:보충과제" />
						</c:when>
					</c:choose>
				</td>
...
<c:if test="${finalExamEvaluateScore > 0 }">
	<div class="vspace"></div>
	<div class="lybox-tbl"> 
		<h4 class="title"><spring:message code="필드:성적관리:기말고사" /></h4> 
	</div>
	<table class="tbl-list">
		<colgroup>
			<col style="width: 90px;" />
		...
		<tbody>
		<c:forEach var="row" items="${applyElementList}" varStatus="i">
			<c:if test="${row.activeElement.referenceTypeCd eq CD_COURSE_ELEMENT_TYPE_FINALEXAM }">
			<tr>
				<td>
					<c:choose>
						<c:when test="${row.activeElement.courseWeekTypeCd eq CD_COURSE_WEEK_TYPE_FINALEXAM and row.applyElement.basicSupplementCd eq CD_BASIC_SUPPLEMENT_BASIC }">
							<aof:code type="print" codeGroup="COURSE_ELEMENT_TYPE" selected="${row.activeElement.referenceTypeCd }" />
						</c:when>
						<c:when test="${row.activeElement.courseWeekTypeCd eq CD_COURSE_WEEK_TYPE_FINALEXAM and row.applyElement.basicSupplementCd eq CD_BASIC_SUPPLEMENT_SUPPLEMENT }">
							<spring:message code="필드:성적관리:보충시험" />
						</c:when>
						<c:when test="${row.activeElement.courseWeekTypeCd eq CD_COURSE_WEEK_TYPE_FINALHOMEWORK and row.applyElement.basicSupplementCd eq CD_BASIC_SUPPLEMENT_BASIC }">
							<spring:message code="필드:성적관리:대체과제" />
						</c:when>
						<c:when test="${row.activeElement.courseWeekTypeCd eq CD_COURSE_WEEK_TYPE_FINALHOMEWORK and row.applyElement.basicSupplementCd eq CD_BASIC_SUPPLEMENT_SUPPLEMENT }">
							<spring:message code="필드:성적관리:보충과제" />
						</c:when>
					</c:choose>
				</td>

...
<c:set var="CD_MIDDLE_FINAL_TYPE_EXAM"   value="${aoffn:code('CD.MIDDLE_FINAL_TYPE.EXAM')}"/>
...
doInitializeLocal = function() {
	...
	forListdata.config.url    = "<c:url value="/univ/course/active/exampaper/list.do"/>";
	
	forInsert = $.action("submit", {formId : "FormInsert"}); // validator를 사용하는 action은 formId를 생성시 setting한다
	forInsert.config.url             = "<c:url value="/univ/course/active/exampaper/insert.do"/>";
...
setValidate = function() {
	...
	forInsert.validator.set({
        title : "<spring:message code="필드:시험:종료기간"/>",
        name : "endDate",
        data : ["!null"],
        check : {
            gt : {name : "startDate", title : "<spring:message code="필드:시험:시작기간"/>"}
        },
        when : function() {
			var courseType = '<c:out value="${courseType}" />';
			return courseType == "period" ? true : false;
	...
	forInsert.validator.set({
		title : "<spring:message code="필드:시험:응시시간"/>",
		name : "examTime",
		data : ["!null", "number"],
		check : {
            maxlength : 3
	...
	forInsert.validator.set({
		title : "<spring:message code="필드:시험:진도율" />",
		name : "limitProgress",
		data : ["!null","number"],
		check : {
			le : 100
...
<c:import url="../../include/commonCourseActiveElement.jsp">
    ...
    <c:param name="selectedElementTypeCd" value="${CD_COURSE_ELEMENT_TYPE_EXAM}"/>
</c:import>
...
<div id="tabContainer">
	...
	<form id="FormInsert" name="FormInsert" method="post" onsubmit="return false;">
	    ...
	    <input type="hidden" name="middleFinalTypeCd" value="<c:out value="${CD_MIDDLE_FINAL_TYPE_EXAM}"/>"/>
	    ...
	    <tbody>
	        <tr>
	            ...
	            <td>
	            	<a href="#" onclick="doBrowseQuiz();" class="pop-call-btn" title="<spring:message code="글:시험:시험팝업"/>" >&nbsp;</a>
	        ...
	        <tr>
	            <th>
	                <spring:message code="필드:시험:응시기간"/> <span class="star">*</span>
	            </th>
	            <td>
	            	<c:choose>
	        ...
	        <tr>
	        	<th>
	                <spring:message code="필드:시험:응시시간"/> <span class="star">*</span>
	            </th>
	            <td>
	        ...
	        <tr>
	        	...
	        	<td>
	        		...
	        		<span id="limitArea" style="display: none;">
		        		<spring:message code="필드:시험:진도율"/>&nbsp;
		        		<input type="text" name="limitProgress" class="align-r" style="width: 70px;">&nbsp; %<spring:message code="글:시험:이상"/>
	        		</span>
	        	</td>
	        </tr>
	        <tr>
	            ...
	            <td>
	            	<input type="text" name="retakeScore" class="align-r" style="width: 70px;">&nbsp;<spring:message code="글:시험:점"/>
	            </td>
	        </tr>
	        <tr>
	        	<th>
	            ...
	            <td>
	            	<input type="text" name="retakeCount" class="align-r" style="width: 70px;">&nbsp;<spring:message code="글:시험:회"/>
	            </td>
	        </tr>
	        <tr>
	            <th>

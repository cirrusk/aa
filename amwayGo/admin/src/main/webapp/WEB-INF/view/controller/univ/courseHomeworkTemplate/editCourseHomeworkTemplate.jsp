...
doInitializeLocal = function() {
	...
	forListdata.config.url    = "<c:url value="/univ/course/homework/template/list.do"/>";
	
	forUpdate = $.action("submit", {formId : "FormUpdate"}); // validator를 사용하는 action은 formId를 생성시 setting한다
	forUpdate.config.url             = "<c:url value="/univ/course/homework/template/update.do"/>";
	...
	forDelete.config.url             = "<c:url value="/univ/course/homework/template/delete.do"/>";
...
<body>
	...
	<div style="display:none;">
		<c:import url="srchCourseHomeworkTemplate.jsp"/>
	</div>
	...
	<form id="FormDelete" name="FormDelete" method="post" onsubmit="return false;">
		<input type="hidden" name="templateSeq" value="<c:out value="${detail.homeworkTemplate.templateSeq}"/>"/>
	</form>
	...
	<tbody>
		<tr>
			...
			<td>
				<input type="hidden" name="courseMasterSeq" value="<c:out value="${detail.homeworkTemplate.courseMasterSeq}"/>">
		...
		<tr>
			...
			<td>
				<input type="hidden" name="profMemberSeq" value="<c:out value="${detail.homeworkTemplate.profMemberSeq}"/>"/>
				...
				<input type="checkbox" name="openYn" value="Y" <c:if test="${detail.homeworkTemplate.openYn eq 'Y'}">checked="checked"</c:if>> <spring:message code="필드:템플릿:전체공개"/>
			</td>
		</tr>
		<tr>
			<th><spring:message code="필드:과제:과제제목"/><span class="star">*</span></th>
			<td>
				<input type="text" name="templateTitle" style="width:350px;" value="<c:out value="${detail.homeworkTemplate.templateTitle}"/>">
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				<div id="uploader" class="uploader">
					<c:if test="${!empty detail.homeworkTemplate.attachList}">
						<c:forEach var="row" items="${detail.homeworkTemplate.attachList}" varStatus="i">
		...
		<tr>
			<td colspan="2">
				<textarea name="templateDescription" id="templateDescription" style="width:99%; height:200px"><c:out value="${detail.homeworkTemplate.templateDescription}"/></textarea>
			</td>
		</tr>
		<tr>
			<th><spring:message code="필드:템플릿:사용여부"/><span class="star">*</span></th>
			<td>
				<aof:code name="useYn" type="radio" removeCodePrefix="true" codeGroup="USEYN" selected="${detail.homeworkTemplate.useYn}"></aof:code>
			</td>
		</tr>
	</tbody>
	</table>

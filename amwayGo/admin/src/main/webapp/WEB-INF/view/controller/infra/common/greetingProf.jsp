...
<c:forEach var="row" items="${appMenuList}">
    <c:choose>
        ...
        <c:when test="${row.menu.url eq '/mypage/course/active/lecturer/mywork/list.do'}"> <%-- 마이페이지의 '나의할일' 메뉴를 찾는다 --%>
...
var menuIds = {
    ...
    "mywork" : "<c:out value="${aoffn:encrypt(menuSystemMywork.menuId)}"/>"
};
...
doMoveBoard = function(boardType) {
	...
	} else if(boardType == "mywork") {
		forMoveBoard.config.url    = "<c:url value="/mypage/course/active/lecturer/mywork/list.do"/>";
	}
	forMoveBoard.run();
};
...
doCourseActiveElementMove = function(mapPKs, type){
    var form = UT.getById(forMoveElement.config.formId);
    form.reset();
	UT.copyValueMapToForm(mapPKs, forMoveElement.config.formId);
    form.elements["currentMenuId"].value = menuIds["mywork"];
...
<div class="lybox-title">
	<h4 class="section-title"><spring:message code="글:마이페이지:나의할일"/></h4>
	<div class="right">
		<a href="javascript:void(0);" onclick="doMoveBoard('mywork');">+<spring:message code="글:마이페이지:더보기"/></a>
	</div>
</div>

<div class="scroll-y mb20" style="height:160px;">	 
	<table id="" class="lecture-list"><!-- tbl-board --> 
		<colgroup> 
			<col style="width:12%;" /><!-- width 값은 각 화면 ui 에 따라 수정 --> 
			<col style="" /> 
			<col style="width:12%;" /> 
		</colgroup> 
		<tbody>
		<c:forEach var="row" items="${mywork.itemList}" varStatus="i">
			<tr> 
				<td>
					<c:choose>
		        		...
		        		<c:when test="${row.element.referenceTypeCd eq CD_COURSE_ELEMENT_TYPE_QUIZ}">
		        			<span class="section-btn yellow"><spring:message code="글:마이페이지:퀴즈"/></span>
		        		</c:when>
		        	</c:choose>
				</td> 
				<td>
                    	...
                    	<dd>
                    		...
                    		<c:out value="${row.active.courseActiveTitle}" />
                   			<c:if test="${row.category.categoryTypeCd eq CD_CATEGORY_TYPE_DEGREE}">
                   				- <span><c:out value="${row.active.division}" /><spring:message code="글:마이페이지:분반"/></span>
                   			</c:if>
                    		<span><aof:date datetime="${row.element.startDtime}" /> ~ <aof:date datetime="${row.element.endDtime}" /></span>
                    		<span><spring:message code="글:마이페이지:응시채점자"/></span>
                    		<span><c:out value="${row.applyElement.takeCount}" /> (<c:out value="${row.applyElement.scoreCount}" />)</span>
                    	</dd> 
                	</dl> 
				</td> 
...
<div class="lybox-title">
	<h4 class="section-title"><spring:message code="글:마이페이지:과목QNA"/></h4>
	<div class="right">
			...
			<tr> 
				...
				<c:choose>
					<c:when test="${row.category.categoryTypeCd eq CD_CATEGORY_TYPE_DEGREE}">
						<c:out value="${row.courseActive.courseActiveTitle}" /> - <c:out value="${row.courseActive.division}" /><spring:message code="글:마이페이지:분반"/>
					</c:when>
					<c:otherwise>
						<c:choose>
							<c:when test="${row.courseActive.courseTypeCd eq CD_COURSE_TYPE_PERIOD}">
								[<c:out value="${row.courseActive.periodNumber}" /><spring:message code="글:마이페이지:기"/>]
							</c:when>
						</c:choose>
					...
					<div class="vspace"></div>
					<a href="javascript:void(0);" onclick="doReply({'parentSeq' : '<c:out value="${row.bbs.bbsSeq}"/>'});"><c:out value="${row.bbs.bbsTitle}" /></a>
				</td> 
...
<table class="tbl-layout"><!-- table-layout --> 
	...
	<tbody> 
		<tr> 
			<td class="first">
				<div class="lybox-title">
					<h4 class="section-title"><spring:message code="글:마이페이지:과목공지사항"/></h4>
					<div class="right">
					...
					<c:forEach var="row" items="${notice}" varStatus="i">
						<tr> 
							...
							<c:choose>
								...
								<div class="vspace"></div>
								<a href="#" onclick="doDetail({'bbsSeq' : '${row.bbs.bbsSeq}'},'notice');"><c:out value="${row.bbs.bbsTitle}" /></a>
							</td><!-- subject --> 
							<td>[<aof:date datetime="${row.bbs.regDtime}"/>]</td> 
						</tr> 
					</c:forEach>

...
doInitializeLocal = function() {
	...
	forSearch.config.url    = "<c:url value="/template/email/list.do"/>";
	...
	forDetail.config.url    = "<c:url value="/template/email/detail.do"/>";

	forCreate = $.action();
	forCreate.config.formId = "FormCreate";
	forCreate.config.url    = "<c:url value="/template/email/create.do"/>";
...
<c:import url="/WEB-INF/view/include/breadcrumb.jsp">
	...
	<thead>
		<tr>
			...
			<th><spring:message code="필드:템플릿:설명" /></th>
	...
	<c:forEach var="row" items="${paginate.itemList}" varStatus="i">
		<tr>
			...
			<td class="align-l ellipsis">
				<c:out value="${row.template.templateDescription}" />
			</td>

...
<body>
	...
	<input type="hidden" name="templateSeq" value="<c:out value="${detail.homeworkTemplate.templateSeq}"/>">
	<input type="hidden" name="templateTitle" value="<c:out value="${detail.homeworkTemplate.templateTitle}"/>">
	<input type="hidden" name="templateDescription" value="<c:out value="${detail.homeworkTemplate.templateDescription}"/>">
	...
	<c:if test="${!empty detail.homeworkTemplate.attachList}">
		...
		<tr>
			...
			<td><aof:code type="print" codeGroup="OPEN_YN" removeCodePrefix="true" selected="${detail.homeworkTemplate.openYn}"></aof:code></td>
		</tr>
		<tr>
		...
		<div class="lybox-btn-l">
			<c:if test="${aoffn:accessible(appCurrentMenu.rolegroupMenu, 'D')}">
				<c:choose>
					...
					<c:otherwise>
						<c:if test="${(condition.srchProfMemberSeq eq detail.homeworkTemplate.profMemberSeq) or (memberSeq eq detail.homeworkTemplate.regMemberSeq)}">
							<a href="javascript:void(0)" onclick="doDelete();" class="btn blue"><span class="mid"><spring:message code="버튼:삭제"/></span></a>
						</c:if>
					</c:otherwise>
				</c:choose>

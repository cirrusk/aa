...
doInitializeLocal = function() {
    ...
    forDownloadExcel.config.url             = "<c:url value="/univ/course/active/teamproject/template/excel.do"/>";
    
	forUploadTemplateExcel = $.action();
	forUploadTemplateExcel.config.formId = "FormInsert";
    forUploadTemplateExcel.config.url             = "<c:url value="/univ/course/active/teamproject/excel/save.do"/>";
    ...
    forUploadTemplateExcel.config.fn.complete     = function(result) {
    	...
    	if(result.success > -1){
    		$.alert({
                message : "<spring:message code="글:엑셀:건이등록되었습니다"/>".format({"0" : result.success}),
                button1 : {
                    callback : function() {
    		...
    		$.alert({message : "<spring:message code="글:엑셀:엑셀파일을확인하십시오"/>".format({"0" : result.error})});
    	}
    }
};
...
<body>
   ...
   <form id="FormInsert" name="FormInsert" method="post" onsubmit="return false;">
	<input type="hidden" name="courseTeamProjectSeq" value="<c:out value="${detail.courseTeamProjectSeq}" />">
	<input type="hidden" name="courseActiveSeq" value="<c:out value="${detail.courseActiveSeq}" />"> 
	<input type="hidden" name="projectTeamCount" value="<c:out value="${detail.projectTeamCount}" />">   		
    ...
    <tbody>
        <tr>
            <th><spring:message code="필드:엑셀:첨부파일"/>
            </th>
            <td>
    ...
    <div class="lybox-btn-r">
        <c:if test="${aoffn:accessible(appCurrentMenu.rolegroupMenu, 'E')}">
            <a href="javascript:void(0)" onclick="doDownLoadExcel()" class="btn blue">
                <span class="mid"><spring:message code="버튼:엑셀:탬플릿다운로드" /></span>
            </a>
        </c:if>
    </div>

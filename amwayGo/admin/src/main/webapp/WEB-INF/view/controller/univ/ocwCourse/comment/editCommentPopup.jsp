...
doInitializeLocal = function() {
	...
	forUpdate.config.url             = "<c:url value="/univ/ocw/course/comment/update.do"/>";
...
<body>
	...
	<form id="FormUpdate" name="FormUpdate" method="post" onsubmit="return false;">
		<input type="hidden" name="ocwCommentSeq" value="<c:out value="${detail.ocwComment.ocwCommentSeq}"/>" />
		
		<div>
			<input type="checkbox" name="secretYn" value="Y" <c:if test="${detail.ocwComment.secretYn eq 'Y'}">checked</c:if> />
			<label class="strong"><spring:message code="필드:게시판:비밀글"/></label>
		</div>
		<div class="simple-box align-c">
			<textarea name="description" style="width:98%;height:140px;"><c:out value="${detail.ocwComment.description}"/></textarea>
		</div>
	</form>
	
	<div class="lybox-btn">
		<div class="lybox-btn-r">
			<c:if test="${aoffn:accessible(appCurrentMenu.rolegroupMenu, 'U') or ssMemberSeq eq detail.ocwComment.regMemberSeq}">

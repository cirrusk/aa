...
doInitializeLocal = function() {
    ...
    forExamResultPopup.config.url            = "<c:url value="/univ/course/active/exampaper/result/detail/popup.do"/>";
    forExamResultPopup.config.options.width  = 900;
    forExamResultPopup.config.options.height = 700;
};
/**
 * 목록보기
 */
				...
				<tr>
					...
					<c:choose>
						<c:when test="${row.courseApplyElement.completeYn eq 'Y'}">
							...
							<td>
								<a href="#" onclick="doExamResultPopup({'courseApplySeq' : '<c:out value="${row.courseApplyElement.courseApplySeq}"/>', 'activeElementSeq' : '<c:out value="${row.courseApplyElement.activeElementSeq}"/>',scoreYn : '<c:out value="${empty row.courseActiveExamPaperTarget.scoreYn ? 'W' : row.courseActiveExamPaperTarget.scoreYn}"/>'});" class="btn black"><span class="small"><spring:message code="버튼:시험:보기"/></span></a>
							</td>
						</c:when>
						<c:otherwise>
							<td>

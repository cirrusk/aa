...
doInitializeLocal = function() {
	...
	forListdata.config.url    = "<c:url value="/message/group/addressgroupList.do"/>";
	...
	forUpdateAddressGroup.config.url = "<c:url value="/message/group/popup.do"/>";
	forUpdateAddressGroup.config.options.width  = 500;
	forUpdateAddressGroup.config.options.height = 150;
	forUpdateAddressGroup.config.options.position = "middle";
	...
	forDeletelist.config.url             = "<c:url value="/message/group/member/deletelist.do"/>";
...
doBrowseMember = function() {
	alert("사람찾기팝업");
	FN.doOpenMemoMemberPopup({url:"<c:url value="/member/memo/list/popup.do"/>", title: "<spring:message code="필드:멤버:주소록"/>", callback:"doSubInsert"});
};
...
<%-- 				<a href="javascript:void(0)" onclick="doBrowseMember()" class="btn blue"><span class="mid"><spring:message code="버튼:주소록:그룹원추가" /></span></a> --%>
			</c:if>

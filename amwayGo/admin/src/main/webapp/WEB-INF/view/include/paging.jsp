	...
	<div class="<c:out value="${styleClass}"/>">
		...
		<c:choose>
			<c:when test="${startPage gt pageLimit}">
				<a href="javascript:void(0)" onclick="<c:out value="${func}" escapeXml="false"/>(1);" class="prev"><span>‹‹ </span><spring:message code="버튼:처음"/></a>			
				<a href="javascript:void(0)" onclick="<c:out value="${func}" escapeXml="false"/>(<c:out value="${startPage - 1}"/>);" class="prev"><span>‹ </span><spring:message code="버튼:이전"/></a>			
			</c:when>
			<c:otherwise>
				<a href="javascript:void(0)" onclick="<c:out value="${func}" escapeXml="false"/>(1);" class="prev"><span>‹‹ </span><spring:message code="버튼:처음"/></a>			
				<a href="javascript:void(0)" onclick="<c:out value="${func}" escapeXml="false"/>(<c:out value="${startPage - 1}"/>);" class="prev"><span>‹ </span><spring:message code="버튼:이전"/></a>
<%-- 				<a class="prev cursor-default"><span>‹‹ </span><spring:message code="버튼:처음"/></a>
				<a class="prev cursor-default"><span>‹ </span><spring:message code="버튼:이전"/></a> --%>
			</c:otherwise>
		</c:choose>
		...
		<c:forEach var="page" begin="${startPage}" end="${endPage}" step="1" varStatus="i">
			...
			<c:choose>
				...
				<c:otherwise>
					<a href="javascript:void(0)" onclick="<c:out value="${func}" escapeXml="false"/>(<c:out value="${page}"/>);"><c:out value="${page}"/></a>			
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:choose>
			<c:when test="${lastPage gt endPage}">
				<a href="javascript:void(0)" onclick="<c:out value="${func}" escapeXml="false"/>(<c:out value="${endPage + 1}"/>);" class="next"><spring:message code="버튼:다음"/><span> ›</span></a>			
				<a href="javascript:void(0)" onclick="<c:out value="${func}" escapeXml="false"/>(<c:out value="${lastPage}"/>);" class="next"><spring:message code="버튼:끝"/><span> ››</span></a>			
			</c:when>
			<c:otherwise>
				<a href="javascript:void(0)" onclick="<c:out value="${func}" escapeXml="false"/>(<c:out value="${endPage + 1}"/>);" class="next"><spring:message code="버튼:다음"/><span> ›</span></a>			
				<a href="javascript:void(0)" onclick="<c:out value="${func}" escapeXml="false"/>(<c:out value="${lastPage}"/>);" class="next"><spring:message code="버튼:끝"/><span> ››</span></a>				
<%-- 				<a class="next cursor-default"><spring:message code="버튼:다음"/><span> ›</span></a>
				<a class="next cursor-default"><spring:message code="버튼:끝"/><span> ››</span></a> --%>
			</c:otherwise>
		</c:choose>
	</div>

...
var FN = {
	...
	doGoMenu : function(url, menuId, formId, target) {
		...
		if (action.config.formId == "FormActiveParam") {
			var valid = true;
			if (typeof form === "undefined" || typeof form !== "object" ) {
				valid = false; 
			} else {
				var el = form.elements["shortcutCourseActiveSeq"];
				if (typeof el === "undefined" || el.value == "") {
					valid = false;
				}
			}
			if (valid == false) {
				jQuery.alert({
								message : "<spring:message code="글:개설과목:개설과목을선택하십시오"/>",
								button1 : {
									callback : function() {
										form.elements["_shortcutCourseActiveSeq_"].focus();
									}
								}
							});
				return;
	...
	/**
	  ...
	  * @param url
	  */
	 doGoFavoriteMenu : function(url, menuId, courseActiveSeq, formId, courseMasterSeq) {
	...
	doDetailMemberPopup : function(param) {
		...
		action.config.formId = "FormParameters";
		action.config.formEmpty = true;
		action.config.url = param.url == null ? "<c:url value="/member/Agreement/popup.do"/>" : param.url;
		var agreeParam = [];
		var courseActiveSeqs = param.srchCourseActiveSeq;
		var memberSeq = param.memberSeq;
					
		if(typeof courseActiveSeqs !== "undefined"){
			agreeParam.push("srchCourseActiveSeq=" + courseActiveSeqs);
		}
		if (typeof memberSeq !== "undefined") {
			agreeParam.push("memberSeq=" + memberSeq);
		}
		action.config.parameters = agreeParam.join("&");
		action.config.options.width = 1000;
		action.config.options.height = 500;
		action.config.options.title = param.title;
		action.run();
	 ...
	 * 첨부파일 다운로드
	 */
	doAttachDownload : function(attachSeq, filename) {
		var action = $.action();
		action.config.formId = "FormParameters";
	...
	doMemoCreate : function(formId, callback) {
		...
		forMemoCreate.config.url = "<c:url value="/message/group/send/popup.do"/>";
	...
	/**
 	 * 회원 이메일 전송 레이어 호출
 	 * 넘겨진 form에 기본 필수 hidden 값:
 	 								체크박스로 선택하여 여러사람한테 보내는 경우 : checkkeys, memberSeqs, memberNames
 	 								한 사람한테 보낼시 사용시 (회원정보) : memberSeq, memberName
 	 								체크박스로 선택하여 그룹대상으로 보내는 경우 : checkkeys, addressGroupSeqs, groupNames
	...
	doCreateEmail : function(formId, callback) {
	    ...
	    forCreateEmail.config.url            = "<c:url value="/message/email/popup.do"/>";
	...
	/**
 	 * 회원 SMS 전송 레이어 호출
 	 * 넘겨진 form에 기본 필수 hidden 값:
 	 								체크박스로 선택하여 여러사람한테 보내는 경우 : checkkeys, memberSeqs, memberNames, phoneMobiles
 	 								한 사람한테 보낼시 사용시 (회원정보) : memberSeq, memberName, phoneMobile
 	 								체크박스로 선택하여 그룹대상으로 보내는 경우 : checkkeys, addressGroupSeqs, groupNames
 	 * callback을 사용할시 예 : FN.doCreateSms(formId, callback);
 	 * callback을 사용안할시 예 : FN.doCreateSms(formId);
	 */
	doCreateSms : function(formId, callback) {
		forCreateSms = $.action("layer", {formId : formId});
	    forCreateSms.config.url            = "<c:url value="/message/sms/popup.do"/>";
	...
	doCreatePush : function(formId, callback) {
        ...
        forCreatePush.config.url            = "<c:url value="/message/push/popup.do"/>";
        forCreatePush.config.options.width  = 500;
        forCreatePush.config.options.height = 300;
        forCreatePush.config.options.position = "middle";
        forCreatePush.config.options.title  = "<spring:message code="글:push:push발송"/>";
        if (typeof callback !== "undefined"){

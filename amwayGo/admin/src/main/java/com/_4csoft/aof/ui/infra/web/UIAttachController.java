...
public class UIAttachController extends BaseController {
	...
	public ModelAndView saveFile(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		try {
			...
			if (file != null && file.getSize() > 0) {
				...
				checkMimeType(file.getInputStream());
	 ...
	 * @throws Exception
	 */
	@RequestMapping ("/attach/contents/save.do")
	public ModelAndView saveContents(HttpServletRequest req, HttpServletResponse res) throws Exception {
		ModelAndView mav = new ModelAndView();
		...
		try {
			...
			if (file != null && file.getSize() > 0) {
				...
				String saveName = StringUtil.getRandomString(20) + extension;
				String savePath = Constants.DIR_TEMP + "/" + DateUtil.getToday("yyyy/MM/dd");
				File saveDir = new File(Constants.UPLOAD_PATH_LCMS + savePath);
				if (saveDir.exists() == false) {
					saveDir.mkdirs();
				}
				saveFile = new File(saveDir.getAbsolutePath() + "/" + saveName);
	 ...
	 * @throws Exception
	 */
	public void checkMimeType(InputStream inputStream) throws Exception {
		List<Object> whiteList = config.getList("upload.mimeTypes.white");

		String mimeType = FileUtil.mimeType(inputStream);

		if (whiteList.indexOf(mimeType) < 0) {
			throw new AofException(Errors.PROCESS_FILE.desc);
		}

		List<Object> blackList = config.getList("upload.mimeTypes.black");

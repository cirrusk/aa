...
var doSearch = function() {
	$("#myContentForm").attr("action", "/lms/myAcademy/lmsMyDeposit.do");
...
var deleteDeposit = function( courseidVal ) {
	if( !confirm("해당 자료를 보관함에서 삭제하시겠습니까?") ) {
		return;
	}
	...
	$.ajaxCall({
   		url: "/lms/myAcademy/lmsMyDepositDeleteAjax.do"
   		, data: param 
   		, dataType: "json"
...
<input type="hidden" name="searchClickYn" value="Y" />
		...
		<section id="pbContent" class="academyWrap">
			<div class="hWrap">
				<h1><img src="/_ui/desktop/images/academy/h1_w030100400.gif" alt="보관함"></h1>
				<p><img src="/_ui/desktop/images/academy/txt_w030100400.gif" alt="보관함에 담은 온라인강의, 교육자료를 확인 할 수 있습니다."></p>
			</div>
						...
						<c:choose>
							<c:when test="${ null ne dataList and !empty dataList }">
								...
								<div class="item <c:if test="${item.viewflag eq 'Y' }">selected</c:if>">
									...
									<a href="#" class="itemDelet" onclick="javascript:deleteDeposit('${item.courseid}');">삭제</a>
								</div>
								</c:forEach>
							</c:when>
							<c:otherwise>

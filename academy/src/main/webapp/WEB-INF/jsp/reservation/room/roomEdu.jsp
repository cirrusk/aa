...
$(document).ready(function () {
	...
	$(".reservationCalendar").click(function () {
		roomConfirmPop();
	});
...
// 		$("#roomEduForm").submit();
		...
		location.href = "<c:url value='/reservation/roomEduForm.do'/>";
	});
...
function setPpInfoList(){
	...
	$.ajaxCall({
		url: "<c:url value='/reservation/roomEduPpInfoListAjax.do'/>"
		, type : "POST"
		, async : false
		, success: function(data, textStatus, jqXHR){
			...
			for(var i = 0; i < ppList.length; i++){
				if(ppList[i].ppseq == lastPp.ppseq){
					html += "<a href='javascript:void(0);' class = 'on' id='detailPpSeq" + ppList[i].ppseq +"' naem=\"detailPpSeq\" onclick=\"javascript:setRoomInfoList('"+ ppList[i].ppseq +"','"+ ppList[i].ppname +"');\">"+ppList[i].ppname+"</a>";
...
function setRoomInfoList(getPpseq, getPpname){
	...
	/* pp버튼 클릭시 선택 테두리 활성화*/
	$("#detailPpSeq"+getPpseq).addClass("on");
	this.ppSeq = getPpseq;
				...
				if(today.ymd > calendar[cnt].ymd){
					...
					for(var j in reservationInfoList){
						if(reservationInfoList[j].ymd == calendar[cnt].ymd
							...
							temp2 = reservationInfoList[j].temp2;
						}
						if(reservationInfoList[j].ymd == calendar[cnt].ymd
					...
					if(workCode == "S02"){
						html += "<td class=\"late\">"
							...
							+ calendar[cnt].day
							+ "<span><em>휴무</em></span>";
					}else if((setCode == null && temp2 == null) || totalCnt == 0){
						html += "<td class=\"late\">"
							+ "<a href=\"javascript:void(0);\" id=\"cal"+calendar[cnt].ymd+"\">"
							+ calendar[cnt].day;
					...
					}else if(temp2 != null || setCode != null){
		...
		, success: function(data, textStatus, jqXHR){
			if(data){
				...
				for (var cnt = 0; cnt < paymentElememtJson.length ; cnt++){
					...
					paymentElememtInput += "<input type='hidden' name='ppseq' 			  value='" + paymentElememtJson[cnt].ppSeq				 + "' />";
					paymentElememtInput += "<input type='hidden' name='typeseq'			  value='" + paymentElememtJson[cnt].typeSeq			 + "' />";
					paymentElememtInput += "<input type='hidden' name='ppname' 			  value='" + paymentElememtJson[cnt].ppName				 + "' />";
					paymentElememtInput += "<input type='hidden' name='cookmastercode'	  value='" + paymentElememtJson[cnt].cookMasterCode		 + "' />";
					paymentElememtInput += "<input type='hidden' name='sessionname' 	  value='" + paymentElememtJson[cnt].sessionName	 	 + "' />";
					paymentElememtInput += "<input type='hidden' name='reservationdate'   value='" + paymentElememtJson[cnt].reservationDate   	 + "' />";
					paymentElememtInput += "<input type='hidden' name='enddatetime' 	  value='" + paymentElememtJson[cnt].endDateTime	 	 + "' />";
					paymentElememtInput += "<input type='hidden' name='roomname' 		  value='" + paymentElememtJson[cnt].roomName			 + "' />";
					paymentElememtInput += "<input type='hidden' name='rsvsessionseq' 	  value='" + paymentElememtJson[cnt].rsvSessionSeq		 + "' />";
					paymentElememtInput += "<input type='hidden' name='paymentstatuscode' value='" + paymentElememtJson[cnt].paymentStatusCode   + "' />";
					paymentElememtInput += "<input type='hidden' name='startdatetime' 	  value='" + paymentElememtJson[cnt].startDateTime	 	 + "' />";
					paymentElememtInput += "<input type='hidden' name='price' 			  value='" + paymentElememtJson[cnt].price			 	 + "' />";
					paymentElememtInput += "<input type='hidden' name='sessionymd' 		  value='" + paymentElememtJson[cnt].sessionYmd		 	 + "' />";
					paymentElememtInput += "<input type='hidden' name='sessionprice' 	  value='" + paymentElememtJson[cnt].sessionPrice	 	 + "' />";
					paymentElememtInput += "<input type='hidden' name='roomseq' 		  value='" + paymentElememtJson[cnt].roomSeq		 	 + "' />";
					paymentElememtInput += "<input type='hidden' name='standbynumber' 	  value='" + paymentElememtJson[cnt].standByNumber	 	 + "' />";
...
/* 
//KICC PROCESSOR
function roomEduPaymentInfoEasyPay(){
	
	$.ajax({
		  url: "<c:url value='/reservation/roomEduReservationInsertAjax.do'/>"
		, type : "POST"
		, data: $("#roomEduAppendCardForm").serialize()
		, success: function(data, textStatus, jqXHR){
			$("#resultKiccProcess").val('true');
		},
		error: function( jqXHR, textStatus, errorThrown) {
			var mag = '<spring:message code="errors.load"/>';
			alert(mag);
...
<section id="pbContent" class="bizroom">
	...
	<div class="hWrap">
		<h1><img src="/_ui/desktop/images/academy/h1_w02_aproom.gif" alt="교육장예약"></h1>
		<p><img src="/_ui/desktop/images/academy/txt_w02_aproom.gif" alt="전국 Amway Plaza에서 운영되고 있는 교육장 이용의 예약 신청을 하실 수 있습니다."></p>
	</div>
	<div class="brIntro">
		<!-- @edit 20160701 인트로 토글 링크영역 변경 -->
		<h2 class="hide">교육장 예약 필수 안내</h2>
		<a href="#uiToggle_01" class="toggleTitle"><strong>교육장 예약 필수 안내</strong> <span class="btnArrow"><em class="hide">내용보기</em></span></a>
						...
						<table class="tblInput">
							...
							<tbody>
								...
								<tr>
									...
									<td>
										...
										<input type="radio" id="radio2" name="paymentmode" value="normal" checked="checked" class="radio2" /><label id="paymentMode-2" for="radio2">일반결제</label>
									</td>
								</tr>
							</tbody>
							...
							<tbody id="normalPayment">
								...
								<tr>
									...
									<td><input type="text" 	   id="cardnumber1" name="cardnumber1" size="4" title="카드번호 첫번째 4자리 입력" maxlength="4" style="ime-mode:disabled" onkeydown='return stringUtility.onlyNumber(event)' onkeyup='stringUtility.removeChar(event)' autocomplete="nope" /> -
										<input type="password" id="cardnumber2" name="cardnumber2" size="4" title="카드번호 두번째 4자리 입력" maxlength="4" style="ime-mode:disabled" onkeydown='return stringUtility.onlyNumber(event)' onkeyup='stringUtility.removeChar(event)' autocomplete="new-password" /> -
										<input type="password" id="cardnumber3" name="cardnumber3" size="4" title="카드번호 세번째 4자리 입력" maxlength="4" style="ime-mode:disabled" onkeydown='return stringUtility.onlyNumber(event)' onkeyup='stringUtility.removeChar(event)' autocomplete="new-password" /> -
										<input type="text"     id="cardnumber4" name="cardnumber4" size="4" title="카드번호 네번째 4자리 입력" maxlength="4" style="ime-mode:disabled" onkeydown='return stringUtility.onlyNumber(event)' onkeyup='stringUtility.removeChar(event)' autocomplete="nope" />
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td><label for="cardpw">비밀번호 앞 2자리</label> 
										<input type="password" size="2" maxlength="2" id="cardpassword" name="cardpassword" style="ime-mode:disabled"
											onkeydown='return stringUtility.onlyNumber(event)' onkeyup='stringUtility.removeChar(event)' autocomplete="new-password" />
									</td>
								</tr>
								<tr>
									<th scope="row" class="thVerticalT">할부기간</th>
								...
								<tr>
									...
									<td>
										<input type="text" size="6" maxlength="2" id="birthday" name="birthday" style="ime-mode:disabled" 
											onkeydown='return stringUtility.onlyNumber(event)' onkeyup='stringUtility.removeChar(event)' autocomplete="new-password" /> 
										<label for="birthday">(생년월일은 주민번호 앞 6자리와 동일하게 입력)</label>
									</td>
								</tr>
								<tr>
									<th scope="row" class="thVerticalT">사업자등록번호</th>
									<td>
										<input type="text" size="10" maxlength="2" id="biznumber" name="biznumber" style="ime-mode:disabled" 
											onkeydown='return stringUtility.onlyNumber(event)' onkeyup='stringUtility.removeChar(event)' autocomplete="new-password" />
										<label for="biznumber">(사업자등록번호 10자리 '-'없이 입력)</label>
									</td>
								</tr>
							</tbody>
						</table>

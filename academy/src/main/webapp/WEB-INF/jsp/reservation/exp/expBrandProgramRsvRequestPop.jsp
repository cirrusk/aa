...
$(document.body).ready(function(){
	...
	$(".btnBasicBL").on("click", function(){
		...
		$.ajax({
			url: "<c:url value='/reservation/expBrandProgramInsertAjax.do' />"
			, type : "POST"
			, data: param
			, success: function(data, textStatus, jqXHR){
				...
				if("false" == data.possibility){
					...
					if(expBrandProgramList[0].msg == "false"){
						alert(expBrandProgramList[0].reason);
						return false;
					}else{
						/* 부모창으로  예약한 데이터 전달 */
						try{
							window.opener.$("#transactionTime").val(data.transactionTime);
							window.opener.expBrandProgramRsvDetail(expBrandProgramList, totalCnt);
							self.close();
						}catch(e){
							//console.log(e);
						}
...
<body>
<form id="expBrandProgramReqForm" name="expBrandProgramReqForm" method="post">
	<div id="pbPopWrap">
	<header id="pbPopHeader">
		<h1><img src="/_ui/desktop/images/academy/h1_w020500080.gif" alt="예약정보 확인" /></h1>
	</header>
	<a href="javascript:void(0);" class="btnPopClose"><img src="/_ui/desktop/images/common/btn_close.gif" alt="팝업창 닫힘" /></a>
	<section id="pbPopContent">
			
		<span class="resvItem">분당ABC<em>|</em>${expBrandProgramList[0].productName}</span>
		...
		<table class="confirmTbl">
			...
			<tbody>
			<c:forEach items="${expBrandProgramList}" var="item">
				<tr>
					...
					<td>
						${item.setDateFormat} ${item.korWeek}
						...
						<input type="hidden" name="setDateFormat" value="${item.setDateFormat}">
						<input type="hidden" name="korWeek" value="${item.korWeek}">

...
$(document.body).ready(function(){
	...
	$(".btnBasicGL").on("click", function(){
		...
		location.href = "<c:url value='${pageContext.request.contextPath}/reservation/expHealthForm.do'/>";
	});
});

function selectCheckedPp(){
// 	console.log($("#ppAppend").children().find("on"));
// 	console.log($("#ppAppend").children("on"));
// 	console.log($("#ppAppend").children().is(".on"));
	if($("#ppAppend").children().length == 0 || $("#ppAppend").children().is(".on") == false){
		alert("PP를 선택해 주십시오.");
		...
		$("#step2Btn").parents('.brWrap').find('.sectionWrap').stop().slideUp(function(){
			...
			$("#ststep2Btnep1Btn").parents('.brWrap').find('.modifyBtn').show();
...
function setPpInfoList(){
	$.ajaxCall({
		url: "<c:url value="/reservation/searchPpInfoListAjax.do"/>"
		, type : "POST"
		, async : false
		, success: function(data, textStatus, jqXHR){
			...
			for(var i = 0; i < ppList.length; i++){
				/* 사용자가 마지막으로 얘약한 pp 디폴트 값 */
				if(ppList[i].ppseq == lastPp.ppseq){
					html += "<a href='javascript:void(0);' class = 'on' id='detailPpSeq" + ppList[i].ppseq +"' onclick=\"javascript:detailPp('"+ppList[i].ppseq+"', '"+ppList[i].ppname+"', '"+ppList[i].expseq+"');\">"+ppList[i].ppname+"</a>";
// 					html += "<a href='javascript:void(0);' id='detailPpSeq" + ppList[i].ppseq +"' onclick=\"javascript:detailPp('"+ppList[i].ppseq+"', '"+ppList[i].ppname+"', '"+ppList[i].expseq+"');\">"+ppList[i].ppname+"</a>";
					...
					/* 상세 pp정보 조회 */
					detailPp(ppList[i].ppseq, ppList[i].ppname, ppList[i].expseq);
				}else{
					/* 첫번째 pp 디폴트 */
					html += "<a href='javascript:void(0);' id='detailPpSeq" + ppList[i].ppseq +"' onclick=\"javascript:detailPp('"+ppList[i].ppseq+"', '"+ppList[i].ppname+"', '"+ppList[i].expseq+"');\">"+ppList[i].ppname+"</a>";
				}
...
// 		this.expseq = getExpseq;
	...
	$.ajaxCall({
		url: "<c:url value='/reservation/expHealthDetailInfoAjax.do' />"
		, type : "POST"
		, data: param
		, success: function(data, textStatus, jqXHR){
			...
			$("#afterProductName").append(detailCode.productname + "-<br/>" + detailCode.content);
			
// 			console.log(detailCode);
			
			if(detailCode.seatcount2 == null||detailCode.seatcount2 == "0"||detailCode.seatcount2 == ""||detailCode.seatcount2 == "undefined"){
				$("#afterSeatcount").append("개인:" + detailCode.seatcount1 + "<br/>");
			}else{
				$("#afterSeatcount").append("개인:" + detailCode.seatcount1 + "<br/>" + "(단체:"+detailCode.seatcount2+")");
			}
			...
			if(detailCode.usetimenote == null || detailCode.usetimenote == "" || detailCode.usetimenote == "undefined"){
				...
				$("#afterUseTime").append(detailCode.usetime + "<br/>" + "("+detailCode.usetimenote+")");				
			}
...
function setFileKeyList(filekey){
	...
	$.ajaxCall({
		url: "<c:url value='/reservation/searchExpHealthFileKeyListAjax.do' />"
		, type : "POST"
		, data: param
		, success: function(data, textStatus, jqXHR){
...
// 					html +="		<img src='/reservation/imageView.do?filefullurl="+imageFileKeyList[i].filefullurl+"&storefilename="+imageFileKeyList[i].storefilename+"'>";
					html +="		<img src='/reservation/imageView.do?file="+imageFileKeyList[i].storefilename+"&mode=RESERVATION' title='"+imageFileKeyList[i].altdesc+"' />";
					html +="	</li>";
				}
...
function nextMonthCalendar(flag){
	...
	$.ajaxCall({
		url: "<c:url value="/reservation/nextMonthCalendarAjax.do"/>"
		, type : "POST"
		, data: param
		, success: function(data, textStatus, jqXHR){
...
function calendarSetData(hoilDayList){
	...
	for(var i = 1; i < 32; i++){
		...
		for(var j = 0; j < hoilDayList.length; j++){
			if($("#cal"+yymm+tempI).text().replace(/\s/gi, '') == Number(hoilDayList[j].setdate)){
...
// 		if(rsvAbleCntList[i].gettoday < rsvAbleCntList[i].getymd && $("#cal"+rsvAbleCntList[i].getymd).children("span").text() == ""){
		if($("#cal"+rsvAbleCntList[i].getymd).children("span").text() == ""){
			if(rsvAbleCntList[i].rsvsessiontotalcnt != 0){
				...
				html = "<em style='color: #555;'> ("+rsvAbleCntList[i].rsvsessiontotalcnt+")</em>";
				$("#cal"+rsvAbleCntList[i].getymd).append(html);
// 				if(worktypecode == rsvAbleCntList[i].worktypecode){
// 					html = "<em style='color: #555;'> ("+rsvAbleCntList[i].rsvsessiontotalcnt+")</em>";
// 					$("#cal"+rsvAbleCntList[i].getymd).append(html);
// 				}else{
...
// 				}
				...
				if(rsvAbleCntList[i].rsvsessiontotalcnt == 0 && rsvAbleCntList[i].rsvsessioncnt == 0 && rsvAbleCntList[i].totalsessioncnt == 0){
	...
	/* 예약 가능 일자가 아닐 경우 alert창 노출 */
	if(this.sessionCnt == Number(rsvAbleCntList[0].getlastday)){
...
function viewRsvConfirm(){
	...
	var frm = document.expHealthForm
...
function setCalendarClassName(yearMonthList) {
	...
	for(var i = 0; i < yearMonthList.length; i++){
		...
		if(i == 0){
			html += "	<span><a href='javascript:void(0);' class='"+yearMonthList[i].engmonth+" on' onclick=\"javascript:changeMonth(this, '"+yearMonthList[i].year+"', '"+yearMonthList[i].month+"');\">"+yearMonthList[i].month+"월</a></span>";
		}else{
		html += "	<span><a href='javascript:void(0);' class='"+yearMonthList[i].engmonth+"' onclick=\"javascript:changeMonth(this, '"+yearMonthList[i].year+"', '"+yearMonthList[i].month+"');\">"+yearMonthList[i].month+"월</a></span>";
		}
	}
	html += "</div>";
...
function reservationReq(){
	...
	$("a[name=startSession]").each(function(index) {
		...
		if($(this).is(".on") === true){
			...
			$("#expHealthFormForPopup").append("<input type= 'hidden' class=\""+$(this).find("input[name='tempExpSessionseq']").val()+$(this).find("input[name='tempReservationDate']").val()+"\" name = 'expsessionseq' value = '"+$(this).find("input[name='tempExpSessionseq']").val()+"'>");
			$("#expHealthFormForPopup").append("<input type= 'hidden' class=\""+$(this).find("input[name='tempExpSessionseq']").val()+$(this).find("input[name='tempReservationDate']").val()+"\" name = 'sessionTime' value = '"+$(this).find("input[name='tempSessionTime']").val()+"'>");
			$("#expHealthFormForPopup").append("<input type= 'hidden' class=\""+$(this).find("input[name='tempExpSessionseq']").val()+$(this).find("input[name='tempReservationDate']").val()+"\" name = 'rsvflag' value = '"+$(this).find("input[name='tempRsvflag']").val()+"'>");
			
			$("#expHealthFormForPopup").append("<input type= 'hidden' class=\""+$(this).find("input[name='tempExpSessionseq']").val()+$(this).find("input[name='tempReservationDate']").val()+"\" name = 'startdatetime' value = '"+$(this).find("input[name='tempStartdatetime']").val()+"'>");
			$("#expHealthFormForPopup").append("<input type= 'hidden' class=\""+$(this).find("input[name='tempExpSessionseq']").val()+$(this).find("input[name='tempReservationDate']").val()+"\" name = 'enddatetime' value = '"+$(this).find("input[name='tempEnddatetime']").val()+"'>");
			
			$("#expHealthFormForPopup").append("<input type= 'hidden' class=\""+$(this).find("input[name='tempExpSessionseq']").val()+$(this).find("input[name='tempReservationDate']").val()+"\" name = 'dateFormat' value = '"+$(this).find("input[name='tempDateFormat']").val()+"'>");
			$("#expHealthFormForPopup").append("<input type= 'hidden' class=\""+$(this).find("input[name='tempExpSessionseq']").val()+$(this).find("input[name='tempReservationDate']").val()+"\" name = 'reservationDate' value = '"+$(this).find("input[name='tempReservationDate']").val()+"'>");
			...
			$("#expHealthFormForPopup").append("<input type= 'hidden' class=\""+$(this).find("input[name='tempExpSessionseq']").val()+$(this).find("input[name='tempReservationDate']").val()+"\" name = 'ppSeq' value = '"+ppSeq+"'>");
			$("#expHealthFormForPopup").append("<input type= 'hidden' class=\""+$(this).find("input[name='tempExpSessionseq']").val()+$(this).find("input[name='tempReservationDate']").val()+"\" name = 'ppName' value = '"+ppName+"'>");
			$("#expHealthFormForPopup").append("<input type= 'hidden' class=\""+$(this).find("input[name='tempExpSessionseq']").val()+$(this).find("input[name='tempReservationDate']").val()+"\" name = 'expseq' value = '"+expseq+"'>");
			$("#expHealthFormForPopup").append("<input type= 'hidden' class=\""+$(this).find("input[name='tempExpSessionseq']").val()+$(this).find("input[name='tempReservationDate']").val()+"\" name = 'typeSeq' value = '"+typeseq+"'>");
...
function expHealthRsvRequestPop() {
	...
	var frm = document.expHealthFormForPopup
	var url = "<c:url value = '/reservation/expHealthRsvRequestPop.do' />";
	var title = "testpop";
...
function expHealthRsvDetail(expHealthRsvList, totalCnt){
	...
	for(var i =0; i < expHealthRsvList.length; i++){
		...
		if(expHealthRsvList[i].partnerTypeCode == "R01"){
			if(expHealthRsvList[i].rsvflag == "100" && expHealthRsvList[i].standbynumber == "0"){
				html += "<p>"+expHealthRsvList[i].dateFormat+"<em>|</em>"+expHealthRsvList[i].sessionTime+"<em>|</em>ABO</p>"
				snsHtml += "■"+expHealthRsvList[i].ppName+"(체성분측정)"+expHealthRsvList[i].dateFormat + "| \n" + expHealthRsvList[i].sessionTime + "| ABO\n";
			}else if(expHealthRsvList[i].rsvflag == "200" || expHealthRsvList[i].standbynumber == "1"){
				html += "<p>"+expHealthRsvList[i].dateFormat+"<em>|</em>"+expHealthRsvList[i].sessionTime+" (예약대기)<em>|</em>ABO</p>"
				snsHtml += "■"+expHealthRsvList[i].ppName+"(체성분측정)"+expHealthRsvList[i].dateFormat + "| \n" + expHealthRsvList[i].sessionTime + " (예약대기) | ABO\n";
			}
			
			
		}else if(expHealthRsvList[i].partnerTypeCode == "R02"){
			if(expHealthRsvList[i].rsvflag == "100" && expHealthRsvList[i].standbynumber == "0"){
				html += "<p>"+expHealthRsvList[i].dateFormat+"<em>|</em>"+expHealthRsvList[i].sessionTime+"<em>|</em>일반인동반</p>"
				snsHtml += "■"+expHealthRsvList[i].ppName+"(체성분측정)"+expHealthRsvList[i].dateFormat + "| \n" + expHealthRsvList[i].sessionTime + "| 일반인동반\n";
			}else if(expHealthRsvList[i].rsvflag == "200" || expHealthRsvList[i].standbynumber == "1"){
				html += "<p>"+expHealthRsvList[i].dateFormat+"<em>|</em>"+expHealthRsvList[i].sessionTime+"(예약대기)<em>|</em>일반인동반</p>"
				snsHtml += "■"+expHealthRsvList[i].ppName+"(체성분측정)"+expHealthRsvList[i].dateFormat + "| \n" + expHealthRsvList[i].sessionTime + " (예약대기) | 일반인동반\n";
			}
		}else if(expHealthRsvList[i].partnerTypeCode == "R03"){
			if(expHealthRsvList[i].rsvflag == "100" && expHealthRsvList[i].standbynumber == "0"){
				html += "<p>"+expHealthRsvList[i].dateFormat+"<em>|</em>"+expHealthRsvList[i].sessionTime+"<em>|</em>비동반</p>"
				snsHtml += "■"+expHealthRsvList[i].ppName+"(체성분측정)"+expHealthRsvList[i].dateFormat + "| \n" + expHealthRsvList[i].sessionTime + "| 비동반\n";
			}else if(expHealthRsvList[i].rsvflag == "200" || expHealthRsvList[i].standbynumber == "1"){
				html += "<p>"+expHealthRsvList[i].dateFormat+"<em>|</em>"+expHealthRsvList[i].sessionTime+"(예약대기)<em>|</em>비동반</p>"
				snsHtml += "■"+expHealthRsvList[i].ppName+"(체성분측정)"+expHealthRsvList[i].dateFormat + "| \n" + expHealthRsvList[i].sessionTime + " (예약대기) | 비동반\n";
			}
		}
...
// 	console.log(param);
	...
	$.ajaxCall({
		url: "<c:url value="/reservation/searchStartSeesionTimeListAjax.do"/>"
		, type : "POST"
		, data: param
		, success: function(data, textStatus, jqXHR){
			var html = "";
			...
			html +=	"<dt id="+sessionTime[0].idvalue+">"+sessionTime[0].date+"<a href= 'javascript:void(0);' class= 'btnDel' onclick='javascript:deleteSessionDetail(this)'>삭제</a></dt>"
			html +=		"<dd class= 'brselectArea'>"
			
			for(var i = 0 ; i < sessionTime.length; i++){
				/* 대기자는 없고 신청된 예약이 존재하면 예약 대기 */
				if(sessionTime[i].rsvflag == 200){
					html +=	"<a href= 'javascript:void(0);' id='"+sessionTime[i].idvalue+"_"+sessionTime[i].expsessionseq+"' name = 'startSession' onclick='javascript:selectedSession(this)' class='line2'>"+sessionTime[i].tempstartdatetime+"<br>(대기신청)"
				...
				/* 대기자도 있고 신청된 예약도 있음 예약 마감 */
					html +=	"<a href= 'javascript:void(0);' id='"+sessionTime[i].idvalue+"_"+sessionTime[i].expsessionseq+"' name = 'startSession' class='line2'>"+sessionTime[i].tempstartdatetime+"<br>(예약마감)</a>"
...
<section id="pbContent" class="bizroom">
	...
	<div class="hWrap">
		<h1><img src="/_ui/desktop/images/academy/h1_w020500160.gif" alt="체성분측정 예약"></h1>
		<p><img src="/_ui/desktop/images/academy/txt_w020500160.gif" alt="뉴트리라이트 체성분 측정을 신청 하실 수 있습니다."></p>
	</div>
	<div class="brIntro">
		<!-- @edit 20160701 인트로 토글 링크영역 변경 -->
		<h2 class="hide">체성분측정 예약 필수 안내</h2>
		<a href="#uiToggle_01" class="toggleTitle"><strong>체성분측정 예약 필수 안내</strong> <span class="btnArrow"><em class="hide">내용보기</em></span></a>
		<div id="uiToggle_01" class="toggleDetail"> <!-- //@edit 20160701 인트로 토글 링크영역 변경 -->
			<c:out value="${reservationInfo}" escapeXml="false" />
		</div>
	</div>
	<div class="brWrapAll">
		<span class="hide">체성분측정 예약 스텝</span>
		<!-- 스텝1 -->
		<div class="brWrap current" id="step1">
			<h2 class="stepTit">
				<span class="close"><img src="/_ui/desktop/images/academy/h2_w02_apexp_01.gif" alt="Step1/지역" class="mglS"/></span>
				<span class="open"><img src="/_ui/desktop/images/academy/h2_w02_apexp_01_current.gif" alt="Step1/지역 - 지역을 선택하세요" class="mglS"/></span>
			</h2>
			<div class="brSelectWrapAll">
				<div class="sectionWrap">
					...
					<section class="brSelectWrap">
						<h3><img src="/_ui/desktop/images/academy/h3_w020500160.gif" alt="프로그램 정보"></h3>
...
<!-- 												<li style="float: none; display: block; position: absolute; top: 0px; left: 228px; width: 228px; height: 148px;"><img src="/_ui/desktop/images/academy/@bodycomposition1.gif" alt="룸 사진"></li> -->
										...
										<div class="sliderPaging"><a href="#none" class="btnPage on"><img src="/_ui/desktop/images/academy/btn_slideb_on.png" alt="룸 사진"></a><a href="#none" class="btnPage"><img src="/_ui/desktop/images/academy/btn_slideb_off.png" alt="룸 사진"></a></div>
									</div>
								</dd>
							</dl>
...
<!-- 									<td>ABO 이상<br/><span class="normal">일반소비자<br/> 동반 가능</span></td> -->
					...
					<div class="btnWrapR">
						<a href="javascript:void(0);" id="step1Btn" class="btnBasicBS stepBtn">다음</a>
					</div>
				</div>
				<div class="result req"><p id="ppName"></p></div>
			</div>
		...
		<div class="brWrap" id="step2">
			<h2 class="stepTit">
				...
				<span class="open"><img src="/_ui/desktop/images/academy/h2_w02_apexp_02_current.gif" alt="Step2/날짜, 시간 - 날짜 선택 후 시간을 선택하세요." /></span>
			</h2>
			<div class="brSelectWrapAll">
				<div class="sectionWrap" id="step2SectionWrap">
					...
					<section class="brSessionWrap">
						<div class="relative">
							...
							<span class="hText">프로그램은 <span id="afterUseTimeTip">30분</span> 단위로 진행됩니다.</span>
						</div>
...
<!-- 							<li id="penaltyInfo">※ 측정 예약 후 <span id="typeValue">7</span>일 전 취소 시 결제 금액의 <strong style="color: red;"><span id="applyTypeValue">10</span>%의 취소 수수료</strong> 제외 후 환불 처리됩니다. (단 예약 당일취소 시 페널티가 적용되지 않습니다.)</li> -->
						</ul>
					</section>
					<div class="btnWrapR">
						<a href="javascript:void(0);" class="btnBasicGS">이전</a>
						<a id="step2Btn" href="javascript:reservationReq();" class="btnBasicBS">예약요청</a>
					</div>
				</div>
				<div class="result" id="tempResult"><p>날짜와 시간을 선택해 주세요.</p></div>
				<div id="expHealthConfirm" class="result lines req" style="display:none">
				<!-- 예약정보 append -->
				</div>
			</div>

	...
	$(".btnBasicBS").on("click", function(){
		...
		if(selectChoice == "resConfirm1"){
			...
			$("input[type='hidden']").each(function(){
				window.opener.$(".tblProgram  input[id='" + $(this).val() + "']").prop("checked", false);
			});
			
			/* 재예약요청 */
			window.opener.reservationReq();
...
<%-- 			<c:out value="expBrandCalendarDisableList"></c:out> --%>
		<c:forEach var="item" items="${expBrandCalendarDisableList}" varStatus="status">

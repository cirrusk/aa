...
function expHealthInsert(){
	...
	var param = $("#expHealthForm").serialize();
	
	$.ajax({
		url: "<c:url value='/reservation/expHealthInsertAjax.do' />"
		, type : "POST"
		, data: param
				...
				if(expHealthRsvList[0].msg == "false"){
					...
					if (totalCnt){
						...
						try{
							...
							window.opener.expHealthRsvDetail(expHealthRsvList, totalCnt);
							self.close();
						}catch(e){
							//console.log(e);
						}
						...
						$(".btnBasicBL").attr("onclick", "javascript:expHealthInsert()");
					}
				}
...
<form id="expHealthForm" name="expHealthForm" method="post">
	...
	<div id="pbPopWrap">
		...
		<section id="pbPopContent">
			...
			<span class="resvItem"><c:out value="${expHealthRsvInfoList[0].ppName}"/><em>|</em>체성분측정</span>
			...
			<table class="confirmTbl">
				...
				<tbody>
					<c:forEach items="${expHealthRsvInfoList}" var="item">
						<tr>
							<td>
								${item.dateFormat}
			...
			<div class="btnWrapC">
				...
				<input type="button" class="btnBasicBL" onclick="javascript:expHealthInsert();" value="예약확정" />
			</div>

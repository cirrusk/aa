...
<script src="/js/front.js"></script>
					<div id="totalCount">${dataList.size()}</div>
					<section id="articleSection" class="acItems">
						...
						<article class="item <c:if test="${item.viewflag eq 'Y' }">selected</c:if>" >
							...
							<div class="snsZone">
								<a href="#none" class="like <c:if test="${item.mylikecnt ne '0'}">on</c:if>" onclick="javascript:likeitemClickDual('${item.courseid}','likecntlabE${item.courseid}','');"><span class="hide">좋아요</span></a>
								<em id="likecntlabE${item.courseid}">${item.likecnt}</em>
								...
								<a href="#none" id="saveitemlabE${item.courseid}" class="save<c:if test="${item.depositcnt ne '0'}"> on</c:if>" onclick="javascript:depositClick('${item.courseid}','saveitemlabE${item.courseid}');"><span class="hide">보관함</span></a>
								<div class="detailSns">

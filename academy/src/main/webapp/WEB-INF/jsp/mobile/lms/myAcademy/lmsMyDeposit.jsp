...
var deleteDeposit = function( thisObj, courseidVal ) {
	if( !confirm("삭제하시겠습니까?") ) {
		return;
	}
	...
	$.ajaxCall({
   		url: "/mobile/lms/myAcademy/lmsMyDepositDeleteAjax.do"
   		, data: param 
   		, dataType: "json"
...
var doSearch = function() {
	$("#myContentForm").attr("action", "/mobile/lms/myAcademy/lmsMyDeposit.do");
	$("#myContentForm").submit();
}
function nextPage(){
	...
	$.ajaxCall({
   		url: "/mobile/lms/myAcademy/lmsMyDepositMoreAjax.do"
   		, data: {page: nextPage}
   		, dataType: "html"
   		, success: function( data, textStatus, jqXHR){
		...
		<section id="pbContent" class="academyWrap">
			...
			<div class="listHead">
				<div class="listSearch">
					총 ${data.totalCount } 개
					<select id="searchcoursetype" name="searchcoursetype" title="보관함 콘텐츠 선택">
				...
				<c:if test="${dataList ne null and dataList.size() > 0}">
					<section id="articleSection" class="acItems">
						...
						<article class="item <c:if test="${item.viewflag eq 'Y' }">selected</c:if>" <c:if test="${status.count > 20}">style="display:none;"</c:if>>
							...
							<a href="#none" class="itemDel" onclick="javascript:deleteDeposit(this,'${item.courseid}');"><span class="hide">삭제</span></a>
						</article>
						</c:forEach>
					</section><!-- //.acItems -->

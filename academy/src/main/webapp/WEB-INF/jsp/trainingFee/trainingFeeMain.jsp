...
<script type="text/javascript">
	$(document.body).ready(function(){
		...
		if( $("#trainingFeeForm > input[name='procType']").val() == "group" ) {
			$("#tabWrapLogical").show();
			//스크롤 사이즈
...
<body>
	...
	<section id="pbContent" class="bizroom">
		...
		<div id="trainingFee">
			...
			<div class="tabWrapLogical" id="tabWrapLogical" style="display:none;">
				<section class="on  logTabSection" id="groupTabClass">
					<h2 class="tab01"><a href="javascript:;" onclick="javascript:tabEvent.groupTab();">그룹</a></h2>
					<div class="tabCont" id="groupTab">
						
						<table class="tblTraningFee" id="tblGroup">
							...
							<tbody>
								...
								<c:forEach var="item" items="${groupList}" varStatus="status">
									<c:if test="${item.giveflag ne 'N'}"> 
										<c:set var="okdata" value="${okdata }, ${item.giveyear}${item.givemonth}"></c:set>
									</c:if>
									<tr class="rowTr" id="grpRow${status.index }">
										<td>${item.giveyear}<br><strong><span class="num">${item.givemonth}</span>월</strong></td>
										<td class="preWon">
											<span class="textPre"><c:if test="${item.trfeetype eq 'NOTFOUND'}"><c:if test="${item.btntype ne 'NOT'}">예상</c:if></c:if></span>
											<span class="num">
											<c:choose>
												<c:when test="${scrData.authgroup eq '0' }">
												</c:when>
												<c:otherwise>
													<c:if test="${!empty item.groupcode}">
														<div id="groupView${status.index }">
															<a href="javascript:;" class="iconGroup" onclick="javascript:listEvent.groupList('${status.index }','${item.giveyear}','${item.givemonth}','${item.groupcode}')"><span class="hide">그룹보기</span></a>
														</div>
														<div id="groupClose${status.index }" style="display:none;">
															<a href="javascript:;" class="iconGroup open" onclick="javascript:listEvent.groupListClose('${status.index }','${item.giveyear}','${item.givemonth}')"><span class="hide">그룹닫기</span></a>
														</div>
													</c:if>
												</c:otherwise>
											</c:choose>
										...
										<c:if test="${item.btntype eq 'PLAN'}">
											<c:choose>
	  											<c:when test="${scrData.authmanageflag eq 'Y' and scrData.giveopen eq 'Y' and item.giveflag ne 'N' }">
													<c:choose>
			  											<c:when test="${item.planstatus eq 'Y'}">
			  												<td class="bg"><a href="#none" class="insertBtn" onclick="javascript:listEvent.planPage('${item.giveyear}','${item.givemonth}','group','S')"><span>계획조회</span></a>
			  													<span class="subText"> 
																	<span><span class="fl">그룹 :</span><span class="fr">${item.plangrpamount }</span></span>
																	<span><span class="fl">차액 :</span><span class="fr">${item.plansumamount }</span></span>
																</span>
			  												</td>
			  											</c:when>
			  											<c:otherwise>
			  												<td class="bg on">
			  													<a href="#none" class="insertBtn on" onclick="javascript:listEvent.planPage('${item.giveyear}','${item.givemonth}','group','I')"><span>${item.plancnt }</span></a>
			  													<span class="subText"> 
																	<span><span class="fl">그룹 :</span><span class="fr">${item.plangrpamount }</span></span>
																	<span><span class="fl">차액 :</span><span class="fr">${item.plansumamount }</span></span>
																</span>
		  										...
		  										<c:when test="${scrData.authmanageflag eq 'Y' and scrData.giveopen eq 'Y' and item.giveflag eq 'N' }">
													<c:choose>
			  											<c:when test="${item.planstatus eq 'Y'}">
		  										...
		  										<c:when test="${scrData.authmanageflag eq 'Y' and scrData.giveopen eq 'N' }">
		  											<c:choose>
			  											<c:when test="${item.planstatus eq 'Y'}">
			  												<td class="bg"><a href="#none" class="insertBtn" onclick="javascript:listEvent.planPage('${item.giveyear}','${item.givemonth}','group','S')"><span>계획조회</span></a></td>
			  											</c:when>
										...
										<c:if test="${item.btntype eq 'ALL'}">
											<c:choose>
	  											<c:when test="${scrData.authmanageflag eq 'Y' and scrData.giveopen eq 'Y' }">
													<c:choose>
			  											<c:when test="${item.planstatus eq 'Y'}">
			  												<td class="bg"><a href="#none" class="insertBtn" onclick="javascript:listEvent.planPage('${item.giveyear}','${item.givemonth}','group','S')"><span>계획조회</span></a>
			  													<span class="subText"> 
			  											...
			  											<c:otherwise>
			  												<td class="bg on"><a href="#none" class="insertBtn on" onclick="javascript:listEvent.planPage('${item.giveyear}','${item.givemonth}','group','I')"><span>${item.plancnt }</span></a>
			  													<span class="subText"> 
																	<span><span class="fl">그룹 :</span><span class="fr">${item.plangrpamount }</span></span>
																	<span><span class="fl">차액 :</span><span class="fr">${item.plansumamount }</span></span>
																</span>
			  										...
			  										<c:choose>
			  											<c:when test="${item.planstatus eq 'Y'}">
			  												<c:choose>
					  											<c:when test="${item.spendstatus eq 'Y'}">
					  												<td class="bg"><a href="#none" class="insertBtn" onclick="javascript:listEvent.spendPage('${item.giveyear}','${item.givemonth}','group','S')"><span>지출조회</span></a>
					  													<span class="subText"> 
																			<span><span class="fl">그룹 :</span><span class="fr">${item.grpamount }</span></span>
																			<span><span class="fl">차액 :</span><span class="fr">${item.spandsumamount }</span></span>
																		</span>
					  												</td>
					  											</c:when>
					  											<c:otherwise>
					  												<td class="bg on"><a href="#none" class="insertBtn on" onclick="javascript:listEvent.spendPage('${item.giveyear}','${item.givemonth}','group','I')"><span>${item.spendcnt }</span></a>
						  												<span class="subText"> 
																			<span><span class="fl">그룹 :</span><span class="fr">${item.grpamount }</span></span>
																			<span><span class="fl">차액 :</span><span class="fr">${item.spandsumamount }</span></span>
																		</span>
			  										...
			  										<c:choose>
			  											<c:when test="${item.planstatus eq 'Y'}">
			  												<c:choose>
					  											...
					  											<c:otherwise>
					  												<td class="bg"><span class="insertPre">-</span></a>
						  												<span class="subText"> 
																			<span><span class="fl">그룹 :</span><span class="fr">${item.grpamount }</span></span>
																			<span><span class="fl">차액 :</span><span class="fr">${item.spandsumamount }</span></span>
																		</span>
											...
											<td>
												<c:choose>
	  												<c:when test="${item.planstatus eq 'Y' and item.spendstatus eq 'Y' }">
	  													<c:choose>
	  														<c:when test="${item.processstatus eq 'Y' }">
	  															<span class="insert end">승인완료<br/>(${item.processstatusdt})</span>
	  														</c:when>
	  														<c:when test="${item.processstatus eq 'R' }">
	  															<span class="insert">반려</span>
																<span class="textTip">${item.rejecttext}</span>
	  														</c:when>
	  														<c:otherwise>
												...
												<span class="insertPre">${item.processstatusname}</span>
											</td>
										</c:if>
										<c:if test="${item.btntype eq 'NOT'}">
											<td class="bg"><span class="insertPre">-</span></td>
											<td class="bg"><span class="insertPre">-</span></td>
											<td><span class="insertPre">-</span></td>
										</c:if>
									...
									<c:choose>
										...
										<c:otherwise>
											<c:forEach var="itemS" items="${groupTargetList}" varStatus="status">
												<c:if test="${item.giveyear eq itemS.giveyear and item.givemonth eq itemS.givemonth }">
												<tr class="groupTr ${itemS.giveyear}${itemS.givemonth}" style="display:none;">
					 								<td></td>
					    							<td class="preWon">
					    								<span class="user">${itemS.depaboname}<br>(${itemS.depaboNo})</span>
					    								<strong class="num">${itemS.trfeetext}</strong>
					    								<c:if test="${scrData.authgroup eq '2'}"><span class="detailView"><a href="javascript:;" onclick="javascript:listEvent.uiLayerPop_w02('${itemS.giveyear}','${itemS.givemonth}','${itemS.depaboNo}')">상세보기</a></span></c:if>
					    							</td>
					    							<td>
					    								<span class="subText">
					    									<span><span class="fl">그룹 :</span><span class="fr">${itemS.plangrpamounttext }</span></span>
					    									<span><span class="fl">개인 :</span><span class="fr">${itemS.planperamounttext }</span></span>
					    									<span><span class="fl">차액 :</span><span class="fr">${itemS.planamountsumtext }</span></span>
					    								</span>
					    							</td>
					    							<td><span class="subText">
					    									<span><span class="fl">그룹 :</span><span class="fr">${itemS.grpamounttext }</span></span>
					    									<span><span class="fl">개인 :</span><span class="fr">${itemS.peramounttext }</span></span>
					    									<span><span class="fl">차액 :</span><span class="fr">${itemS.spandamountsumtext }</span></span>
					    								</span>
					    							</td>
					    							<td></td>
					    						</tr>
				...
				<section class="logTabSection" id="personTabClass">
					<h2 class="tab02"><a href="javascript:;" onclick="javascript:tabEvent.personTab();">개인</a></h2>
					<div class="tabCont" id="personTab">
											...
											<c:choose>
	  											<c:when test="${scrData.giveopen eq 'Y' and itemp.giveflag ne 'N'}">
	  												<c:choose>
			  											...
			  											<c:otherwise>
			  												<td class="bg on">
			  													<a href="#none" class="insertBtn on" onclick="javascript:listEvent.planPage('${itemp.giveyear}','${itemp.givemonth}','person','I')"><span>${itemp.plancnt }</span></a>
			  													<span class="subText"> 
																	<span><span class="fl">그룹 :</span><span class="fr">${itemp.plangrpamount }</span></span>
																	<span><span class="fl">개인 :</span><span class="fr">${itemp.planperamount }</span></span>
																	<span><span class="fl">차액 :</span><span class="fr">${itemp.plansumamount }</span></span>

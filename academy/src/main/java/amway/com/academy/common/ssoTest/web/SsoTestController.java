...
public class SsoTestController {
	...
	public ModelAndView getCodeList(RequestBox requestBox, ModelAndView mav, HttpServletRequest request, HttpServletResponse response)  throws Exception  {
		...
		if( ssoValue.equals("ERROR") ) {
			...
			if( !blankStr.equals(ssoValue) ) {
				...
				// 세션 아웃
				request.getSession().invalidate();
				LOGGER.debug("================== imar값이 없어서 로그아웃 시킴");
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/ssoTokenTest.do")
	public ModelAndView getCodeListToken(RequestBox requestBox, ModelAndView mav, HttpServletRequest request, HttpServletResponse response)  throws Exception  {
		Map<String, String> rMap = new HashMap<String, String>();
		//삼성 하이브리스 연결
		String ssoValue = "";
		String ssoResult = "";
		...
		if( requestUrl.indexOf("http://localhost") >= 0 ) {
			...
			//프로퍼티의 Hybris.ssoUrl정보를 읽어서 https://localhost 이면 체크하지 않는다.
			//String urlString = "https://qa2.abnkorea.co.kr/rest/oauth/token?client_id=1ztr8zUTLBdlj2k&client_secret=8342251a-77d8-43bd-9145-d1fd997483c2&grant_type=client_credentials";
			String urlString = "https://dev2.abnkorea.co.kr/rest/oauth/token?client_id=1ztr8zUTLBdlj2k&client_secret=8342251a-77d8-43bd-9145-d1fd997483c2&grant_type=client_credentials";

...
$(document.body).ready(function(){
	...
	expPenaltyList.init();
	expPenaltyList.doSearch();
	
	// 검색버튼 클릭
	$("#btnSearch").on("click", function(){
		expPenaltyList.doSearch({page:1});
	});
	
	// 페이지당 보기수 변경 이벤트
	$("#cboPagePerRow").on("change", function(){
		expPenaltyList.doSearch({page:1, rowPerPage : $("#cboPagePerRow").val() });
	});
	
	// 패널티 현황 엑셀 다운로드
	$("#aExpPenaltyExcelDown").on("click", function () {
		...
		if(result) {
			...
			postGoto("/manager/reservation/expPenalty/expPenaltyListExcelDownload.do", initParam);
...
/* 시설 패널티 현황 상세 */
function expPenaltyDetailPop(historyseq) {
	var popParam = {
			url : "<c:url value="/manager/reservation/expPenalty/expPenaltyDetailPop.do" />"
			, width : "512"
			, height : "400"
...
/* 시설 패널티 해제 */
function expPenaltyCancelLimit(historyseq, rsvseq) {
	var param = {
	...
	sUrl = "<c:url value="/manager/reservation/expPenalty/expPenaltyCancelLimitUpdateAjax.do"/>";
...
//Grid Init
var expPenaltyListGrid = new AXGrid(); // instance 상단그리드
var expPenaltyList = {
		/** init : 초기 화면 구성 (Grid) */
		init : function() {
			var idx = 0; // 정렬 Index 
			var _colGroup = [
							...
							, {key:"aboname", label:"이름", width:"100", align:"center", sort:false}
							, {key:"temp", label:"회원구분", width:"100", align:"center", sort:false}
							, {key:"account", label:"예약자 번호", width:"150", align:"center", sort:false}
							...
							, {key:"btns", addClass:idx++, label:"상세보기", width:"100", align:"center", formatter: function(){
								return "<a href=\"javascript:;\" class=\"btn_green\" onclick=\"javasctipt:expPenaltyDetailPop('"+this.item.historyseq+"')\">보기</a>";
							  }}
							, {key:"btns", addClass:idx++, label:"해제", width:"100", align:"center", formatter: function(){
								if(this.item.applytypecode != 'P01'){
									if(this.item.penaltystatuscode == 'B02'){
										return "해제완료";
									}else{
										if(param.menuAuth == "W"){
											return "<a href=\"javascript:;\" class=\"btn_green authWrite\" onclick=\"javasctipt:expPenaltyCancelLimit('"+this.item.historyseq+"', '"+this.item.rsvseq+"')\">해제</a>";
										}else{
											return "";
										}
									}
			...
			var gridParam = {
					...
					, sortFunc : expPenaltyList.doSortSearch
					, doPageSearch : expPenaltyList.doPageSearch
				}
			
			fnGrid.initGrid(expPenaltyListGrid, gridParam);
		}, doPageSearch : function(pageNo) {
			// Grid Page List
			expPenaltyList.doSearch({page:pageNo});
			...
			$.extend(defaultParam, initParam);
 		   	...
 		   	$.ajaxCall({
		   		url: "<c:url value="/manager/reservation/expPenalty/expPenaltyListAjax.do"/>"
		   		, data: defaultParam
		   		, success: function( data, textStatus, jqXHR){
		   			if(data.result < 1){
		   				var mag = '<spring:message code="errors.load"/>';
...
// 		   		console.log(obj);
		   		...
		   		// Grid Bind Real
		   		expPenaltyListGrid.setData(gridData);
		   	}
		}
	}
...
<body class="bgw">
	...
	<div class="contents_title clear">
		...
		<div class="fr">
			<a href="javascript:;" id="aExpPenaltyExcelDown" class="btn_green">엑셀다운</a>
		</div>
	</div>

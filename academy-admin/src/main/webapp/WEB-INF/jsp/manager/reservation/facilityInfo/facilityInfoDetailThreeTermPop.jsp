...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("#selsession").hide(); //특정세션 안보이게

        //요리명장 조건
        if($("[name='cookType']").val()=="Y"){
            $("#rsvnormal").hide();
            $("#condition").hide();
            $("#rsvspecial").show();
            $("#cheSpecial").prop("checked",true);
        };
        //------------셀렉트박스 월
        var html=[];
        var value = "";
    ...
    function fnsetweek(num) {
    	$.ajaxCall({
            url: "<c:url value='/manager/reservation/facilityInfo/facilityInfoDetailThreeSessionAjax.do'/>"
            , data: {
                  roomseq : $("#roomseq").val()
                ...
                if(data.result < 1){
                    ...
                    if( data.dataList != null ) {
                        ...
                        for(var i=0;i<data.dataList.length; i++) {
                        	lHtml = lHtml + "<option value='"+ data.dataList[i].rsvsessionseq +"'>"+ data.dataList[i].sessionname +"</option>";
                        }
    ...
    var pop = {
         doSave : function(){
        	if($("[name='apply']:checked").val()=="P01"){
            	// 모든세션
            	var sUrl = "<c:url value="/manager/reservation/facilityInfo/facilityInfoAllSessionInsert.do"/>";
            } else {
            	// 특정세션
            	var sUrl = "<c:url value="/manager/reservation/facilityInfo/facilityInfoDetailThreeTermInsert.do"/>";
            }
//             var sUrl = "<c:url value="/manager/reservation/facilityInfo/facilityInfoAllSessionInsert.do"/>";
            var sMsg = "등록하였습니다.";
            ...
            var masterParam = {
                ...
                , periodtypecode : $("[name='period']:checked").val()
                , startmonth     : "0"//$("#startmonth").val()
                , startday       : startday
                , endmonth       : isNvl(endmonth,"")
                , endday         : endday
                , applysessiontypecode : $("[name='apply']:checked").val()
                , roomseq : $("#roomseq").val()
            };
            ...
            $.ajaxCall({
                ...
                , success: function( data, textStatus, jqXHR){
                    if(data.result.errCode < 0){
                        ...
                        if("Y" == $("[name='cookType']").val()) {
                            eval($('#ifrm_main_'+frmId).get(0).contentWindow.doReturn("cookTermList"));
                        }else{
                            eval($('#ifrm_main_'+frmId).get(0).contentWindow.doReturn("termList"));
                        }
...
<div id="popwrap">
    ...
    <div id="popcontainer" style="height:270px">
        <input type="hidden" id="roomseq" value="${layerMode.roomseq}"/>
        <input type="hidden" name="frmId" value="${layerMode.frmId}"/>
        <input type="hidden" name="cookType" value="${layerMode.cookType}"/>
        <div id="popcontent">
            <!-- Sub Title -->
            ...
            <div class="tbl_write">
                ...
                <div id="area1">
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            ...
                            <td>
                                당일부터
                                <select class="boxday" id="startday" name="startday" style="min-width:50px;">
                                    <option value="">선택</option>
                                </select>
                                일 후
                            </td>
                ...
                <div id="selsession" >
	                <div style="float:right;">
	                    <a href="javascript:;" class="btn_green" id="hang">행추가</a>
	                </div>
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    	<colgroup>
							<col width="15%" />

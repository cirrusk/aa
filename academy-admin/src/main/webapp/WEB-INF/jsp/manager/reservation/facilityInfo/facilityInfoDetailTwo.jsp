        ...
        $("#btnAdd").on("click",function(){
            ...
            inner += '  </td>';
                if(checkCount>1){
                    ...
                    inner += '  </td>';
                    inner += '  <td>';
                    inner += '      정상가 <input type="text" class="AXInput isNum" name="qnNo[]" value="0" style="width:50px;IME-MODE:disabled;" maxlength="4" onkeydown="return showKeyCode(event)" /> 만원<br/>';
                    inner += '      할인가 <input type="text" class="AXInput isNum" name="qnSale[]" value="0" style="width:50px;IME-MODE:disabled;" maxlength="4" onkeydown="return showKeyCode(event)" /> 만원 *할인가가 없을경우0으로 입력해 주십시오.';
                    inner += '  </td>';
                }else{
                 if(typeName != "비즈룸"){
            ...
            inner += '    <td><a href="javascript:;" class="btn_green" name="delStaff">세션삭제</a></td>';
            inner += '</tr>';
        ...
        init : function() {
            ...
            head += '    <th scope="row" style="text-align: center;">시간선택</th>';
                if (checkCount > 1) {
                    ...
                    head += '    <th scope="row" style="text-align: center;">가격(퀸룸/파티룸)</th>';
                } else {
                    if (typeName == "퀸룸/파티룸" || typeName == "퀸룸" || typeName == "파티룸") {
                        head += '    <th scope="row" style="text-align: center;">가격(퀸룸/파티룸)</th>';
                    } else if (typeName == "교육장") {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                inner += '  </td>';
                                    ...
                                    if (checkCount > 1) {
                                        ...
                                        inner += '  </td>';
                                        inner += '  <td>';
                                        inner += '      정상가 <input type="text" class="AXInput" name="qnNo[]" style="width:50px;IME-MODE:disabled;" maxlength="4" onkeydown="return showKeyCode(event)"  value="' + data[i].queenprice / 10000 + '"/> 만원<br/>';
                                        inner += '      할인가 <input type="text" class="AXInput" name="qnSale[]" style="width:50px;IME-MODE:disabled;" maxlength="4" onkeydown="return showKeyCode(event)"  value="' + data[i].queendiscountprice / 10000 + '"/> 만원 *할인가가 없을경우0으로 입력해 주십시오.';
                                        inner += '  </td>';
                                    } else {
                                     if(typeName != "비즈룸") {
                                        if (typeName == "퀸룸/파티룸" || typeName == "퀸룸" || typeName == "파티룸") {
                                ...
                                inner += '    <td><a href="javascript:;" name="delStaff" class="btn_green" >세션삭제</a></td>';
                                inner += '</tr>';
    ...
    var list = {
        ...
        , doSave : function(mode){
            ...
            for(var i=0; i<stHour.length; i++){
                ...
                if (checkCount > 1) {
                    if (i == 0) {
                        weekRow = hiddenseq[i].value + "/" + stTime + "/" + endTime + "/" + isNvl(noNo[i].value, "0") + "/" + isNvl(noSale[i].value, "0") + "/" + isNvl(qnNo[i].value, "0") + "/" + isNvl(qnSale[i].value, "0");
                    } else {
                        weekRow = weekRow + "," + hiddenseq[i].value + "/" + stTime + "/" + endTime + "/" + isNvl(noNo[i].value, "0") + "/" + isNvl(noSale[i].value, "0") + "/" + isNvl(qnNo[i].value, "0") + "/" + isNvl(qnSale[i].value, "0");
                    }
                } else {
                    if (typeName == "퀸룸/파티룸" || typeName == "퀸룸" || typeName == "파티룸") {
                        if (i == 0) {
                            weekRow = hiddenseq[i].value + "/" + stTime + "/" + endTime + "/" + 0 + "/" + 0 + "/" + isNvl(qnNo[i].value, "0") + "/" + isNvl(qnSale[i].value, "0");
                        } else {
                            weekRow = weekRow + "," + hiddenseq[i].value + "/" + stTime + "/" + endTime + "/" + 0 + "/" + 0 + "/" + isNvl(qnNo[i].value, "0") + "/" + isNvl(qnSale[i].value, "0");
                        }
                    } else if (typeName == "교육장") {
                        if (i == 0) {
                        ...
                        if (i == 0) {
                            weekRow = hiddenseq[i].value + "/" + stTime + "/" + endTime + "/" + 0 + "/" + 0 + "/" + 0 + "/" + 0;
                        } else {
                            weekRow = weekRow + "," + hiddenseq[i].value + "/" + stTime + "/" + endTime + "/" + 0 + "/" + 0 + "/" + 0 + "/" + 0;
                        }
                    }
                }
            }

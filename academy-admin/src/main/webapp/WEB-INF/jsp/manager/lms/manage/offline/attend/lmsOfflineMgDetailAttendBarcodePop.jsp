...
function fnBarcodeAttend()
{
	var param = $("#frm").serialize();
	
	
	$.ajaxCall({
		url:"<c:url value="/manager/lms/manage/offline/attend/lmsOfflineMgAttendBarcodeAjax.do"/>"
		,data:param
						...
						{		
							var result = confirm("ABO번호 : "+data.uid+"\r주사업자 : "+data.mainname+"\r주사업자 생년월일 : "+data.ssn +"\r부사업자 : "+data.partnerinfoname+"\r부사업자 생년월일:"+data.partnerinfossn);
								...
								if(result)
								{
									...
									$.ajaxCall({
								   		url: "<c:url value="/manager/lms/manage/offline/attend/lmsOfflineMgAttendBarcodeMemberInfoConfirmAjax.do"/>"
								   		, data: param
								   		, success: function( data, textStatus, jqXHR){
								   			...
								   			if(data.comment != null && data.comment != "")
											{
							   				...
							   				{	
												...
												alert(data.uid)
												alert(data.mainname)
												alert(data.partnerinfoname)
												alert(data.membertype)
												alert(data.seatnumber)
...
$(document.body).ready(function(){
	...
	$("#closeBtn").on("click",function(){
		...
		$("#"+g_managerLayerMenuId.callId).get(0).contentWindow.layer.setViewTab("3",iniObject);
		$("#"+g_managerLayerMenuId.callId).get(0).contentWindow.newShowApplicantCount();
	});
	...
	$("#exceptBtn").on("click",function(){
		...
		$.ajaxCall({
	   		url: "<c:url value="/manager/lms/manage/offline/attend/lmsOfflineMgAttendBarcodeExceptionAjax.do"/>"
	   		, data: param
	   		, success: function( data, textStatus, jqXHR){
		...
		<table class="tblInput">
			...
			<tbody>
  			<tr>
				...
				<td>
				<div class="rela">
					<input type="text" id="uid" name="uid"  style="width:auto; min-width:200px" required="required">

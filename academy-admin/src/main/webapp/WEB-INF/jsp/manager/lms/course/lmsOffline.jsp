	...
	$("#aExcdlDown").on("click", function(){
		...
		if(result) {
			...
			var initParam = {
				...
				, searchpenaltyflag :$("#searchpenaltyflag").val()
			...
			postGoto("<c:url value="/manager/lms/course/lmsOfflineExcelDownload.do"/>", defaultParam);
			hideLoading();
		}
	});	
});
		...
		*/		
		init : function() {
			...
			var _colGroup = [
				...
				, {key:"studentcount", label:"수강생", width:"80", align:"center", sort:false, display:false}
				...
				, {key:"themename", label:"브랜드/테마", width:"200", align:"center", sort:false}
				...
				, {key:"penaltyflagname", label:"페널티", width:"80", align:"center", sort:false}
			...
			$.ajaxCall({
		   		url: "<c:url value="/manager/lms/course/lmsOfflineListAjax.do"/>"
		   		, data: defaultParam
...
var gridEvent = {
	chkDelete : function() {
		...
		for(var i = 0; i<listIdx.length;i++) {
			if( listIdx[i].item.studentcount != 0) {
				if( !confirm(listIdx[i].item.coursename + " 과정에 수강생이 등록되어 있습니다.\n삭제하시겠습니까?")){
					return;	
				}
			}
		}
		...
		var result = confirm("삭제 시 신청자, 수강자 정보와 AmwayGO 그룹방이 모두 삭제됩니다.\n그래도 삭제를 진행하시겠습니까?"); 
		
		if(result) {
			$.ajaxCall({
				url: "<c:url value="/manager/lms/course/lmsOfflineDeleteAjax.do"/>"
...
function goWrite(mode, courseid){
	...
	var strMenuText = "오프라인과정등록";
	var strLinkurl = "/manager/lms/course/lmsOfflineWrite.do";
	// 탭이동시 열려있으면 닫고 이동한다.
	if(window.parent.g_managerLayerMenuId.sessionOnOffcourseid != null && window.parent.g_managerLayerMenuId.sessionOnOffcourseid != courseid){
		window.parent.closeTabMenu(strMenuCd);
	}
	// 전역변수 처리
	window.parent.g_managerLayerMenuId.sessionOnOffcourseid  = courseid;
	...
	<div class="tbl_write">
		<table id="tblSearch" width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<th>페널티</th>
				<td>
					<select id="searchpenaltyflag" name="searchpenaltyflag" style="width:auto; min-width:100px" >
						<option value="">선택</option>

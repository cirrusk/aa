...
$(document.body).ready(function(){
	...
	$("#aExcdlDown").on("click", function(){
		...
		if(result) {
			...
			postGoto("<c:url value="/manager/lms/testMg/lmsTestMgExcelDownload.do"/>", defaultParam);
			hideLoading();
		}
	});	
	...
	*/		
	init : function() {
		...
		var _colGroup = [
			...
			, {key:"categorytreename", label:"시험분류", width:"200", align:"center", sort:false}
			, {key:"coursename", label:"시험지명", width:"350", align:"center", sort:false}
			, {key:"edudate", label:"평가기간", width:"200", align:"center", sort:false}
			, {
		...
		$.ajaxCall({
			url: "<c:url value='/manager/lms/testMg/lmsTestMgAjax.do'/>"
	   		, data: defaultParam
...
var gridEvent = {
	chkDelete : function() {
		...
		var result = confirm("선택한 시험을 삭제 하겠습니까?"); 
		
		if(result) {
			$.ajaxCall({
				url: "<c:url value="/manager/lms/testMg/lmsTestMgDeleteAjax.do"/>"
...
function goWrite(modeVal,courseidVal){
	...
	var popParam = {
		url : "<c:url value='/manager/lms/testMg/lmsTestMgPop.do' />"
		, width : 1020
		, height : 800
		, maxHeight : 800
		, params : param
...
<body class="bgw">
	...
	<div class="tbl_write">
		<table id="tblSearch" width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				<th scope="row">시험분류</th>
				<td>
					<select id="searchcategoryid" name="searchcategoryid" style="width:auto; min-width:200px" >

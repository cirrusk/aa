	...
	// 엑셀다운 버튼 클릭시
 	$("#aExcdlDown").on("click", function(){
		...
		if(result) {
			...
			postGoto("<c:url value="/manager/lms/manage/offline/lmsOfflineMgExcelDownload.do"/>", defaultParam);
			hideLoading();
		}
	});	
});
...
var list = {
		...
		*/		
		init : function() {
			var idx = 0; // 정렬 Index 
			var _colGroup = [
				{key:"courseid", label: "", width: "0", align : "center", sort:false, display:false}
				...
				, {key:"themename", label:"오프라인테마", width:"200", align:"center", sort:false}
				, {key:"coursename", label:"과정명", width:"460", align:"center", sort:false, formatter: function () {
					return "<a href=\"javascript:;\" onclick=\"javascript:goDetail(" + this.item.courseid + ");\">"+this.item.coursename+"</a>";
				}}
				, {key:"apname", label:"교육장소", width:"100", align:"center", sort:false}
				, {key:"edudate", label:"교육기간", width:"200", align:"center", sort:false}
				, {key:"edustatus", label:"상태", width:"80", align:"center", sort:false}
				, {key:"requestcount", label:"신청", width:"80", align:"center", sort:false}
				, {key:"finishcount", label:"수료", width:"80", align:"center", sort:false}
				,{
					key:"detail", label:"상세", width:"80", align:"center", sort:false, formatter: function () {
						return "<a href='javascript:;' class='btn_green' onclick='javascript:goDetail("+this.item.courseid+");'>상세</a>";
					}
				}
			]
			var gridParam = {
			...
			$.extend(defaultParam, initParam);
			$.ajaxCall({
		   		url: "<c:url value="/manager/lms/manage/offline/lmsOfflineMgListAjax.do"/>"
		   		, data: defaultParam
	...
	{	
		...
		// 상세 tabpage
			var strMenuCd   = "W010200101";
			var strMenuText = "오프라인과정운영 상세";
			var strLinkurl = "/manager/lms/manage/offline/lmsOfflineMgDetail.do";
			// 탭이동시 열려있으면 닫고 이동한다.
			if(window.parent.g_managerLayerMenuId.sessionOnOffMgcourseid != null && window.parent.g_managerLayerMenuId.sessionOnOffMgcourseid != courseid){
				window.parent.closeTabMenu(strMenuCd);
			}
			// 전역변수 처리
			window.parent.g_managerLayerMenuId.sessionOnOffMgcourseid  = courseid;
			...
			var item = { 
				...
				, callFn   : 'reloadFrame' 
				, funcData : ''
				, urlParam :"courseid="+courseid+"&prefrmId="+frmid
			};
	...
	<div class="tbl_write">
		<table id="tblSearch" width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td>
					<select id="searchedustatus" name="searchedustatus" style="width:auto; min-width:100px" >
						...
						<c:forEach items="${eduStatusList }" var="eduStatus">
							<option value="${eduStatus.commoncodeseq }">${eduStatus.codename }</option>
						</c:forEach>
					</select>
				</td>
				<th rowspan="4">
			...
			<tr>
				<th>교육장소</th>
				<td>
					<select id="searchapseq" name="searchapseq">
						<option value="">선택</option>
			...
			<tr>
				...
				<td colspan="3">
					<select id="searchtype" name="searchtype" style="width:auto; min-width:100px" >
						...
						<option value="1">오프라인테마명</option>
						<option value="2">과정명</option>
						<option value="3">과정설명</option>
					</select>

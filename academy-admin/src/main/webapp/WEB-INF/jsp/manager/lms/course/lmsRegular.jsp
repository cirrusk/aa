...
$(document.body).ready(function(){
	...
	$("#aExcdlDown").on("click", function(){
		...
		if(result) {
			...
			postGoto("/manager/lms/course/lmsRegularExcelDownload.do", defaultParam);
			hideLoading();
		}
	});	
});
		...
		*/
		init : function() {
			...
			var _colGroup = [
				...
				, {key:"coursename", label:"정규과정명", width:"500", align:"center", sort:false, formatter: function () {
					return "<a href=\"javascript:;\" onclick=\"javascript:goWrite('U', '" + this.item.courseid + "');\">"+this.item.coursename+"</a>";
				}}
				, {key:"edudate", label:"교육기간", width:"240", align:"center", sort:false}
			...
			$.ajaxCall({
		   		url: "/manager/lms/course/lmsRegularListAjax.do"
		   		, data: defaultParam
...
var gridEvent = {
	chkDelete : function() {
		...
		if(result) {
			$.ajaxCall({
				url: "/manager/lms/course/lmsRegularDeleteAjax.do"
...
function goWrite(mode, courseid){
	...
	var strMenuText = "정규과정등록";
	var strLinkurl = "/manager/lms/course/lmsRegularWrite.do";
	// 탭이동시 열려있으면 닫고 이동한다.
	if(window.parent.g_managerLayerMenuId.sessionOnRegularcourseid != null && window.parent.g_managerLayerMenuId.sessionOnRegularcourseid != courseid){
		window.parent.closeTabMenu(strMenuCd);
	}
	// 전역변수 처리
	window.parent.g_managerLayerMenuId.sessionOnRegularcourseid  = courseid;
	...
	//본창에서 등록
	location.href="/manager/lms/course/lmsRegularWrite.do?mode="+mode+"&courseid="+courseid;
	return;
	//팝업창등록
	var param = {
			mode : mode ,
	...
	var popParam = {
			url : "/manager/lms/course/lmsRegularWrite.do"
			, width : 1024
			, height : 800
			, maxHeight : 800
			, params : param

...
function saveSubmit(){
	...
	$.ajaxCall({
   		url : "<c:url value="/manager/lms/course/lmsCourseConditionExcelSaveAjax.do"/>"
   		, timeout : 60 * 1000
   		, data : param
   		, type: 'POST'
...
$(document).ready(function(){
	...
	$("#loacode2").on("change", function() {
		...
		$.ajaxCall({
	   		url: "<c:url value="/manager/lms/course/lmsCourseConditionDiaAjax.do"/>"
	   		, data: param
	   		, success: function( data, textStatus, jqXHR){
	   			if(data.result < 1){
   	           		alert("<spring:message code="errors.load"/>");
	...
	$("#aExcdlDown").on("click", function(){
		...
		if(result) {
			...
			postGoto("<c:url value="/manager/lms/course/lmsCourseConditionSampleExcelDownload.do"/>", defaultParam);
			hideLoading();
		}
	});	
	...
	/* 값이 모두 없는 경우 회원타입 > ABO 기본 설정 해준다. */

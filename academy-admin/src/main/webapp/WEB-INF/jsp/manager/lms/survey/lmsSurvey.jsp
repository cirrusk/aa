	...
	$("#aExcdlDown").on("click", function(){
		...
		if(result) {
			...
			var initParam = {
				...
				, searchsurveystart : $("#searchsurveystart").val()	
				, searchsurveyend :$("#searchsurveyend").val()
				, searchtext :$("#searchtext").val()
			};
			$.extend(defaultParam, initParam);
			postGoto("<c:url value="/manager/lms/survey/lmsSurveyExcelDownload.do"/>", defaultParam);
			hideLoading();
		}
	});	
});
	...
	*/		
	init : function() {
		...
		var _colGroup = [
			...
			, {key:"coursename", label:"설문명", width:"350", align:"center", sort:false}
			, {key:"edudate", label:"설문기간", width:"250", align:"center", sort:false}
			, {
		...
		$.ajaxCall({
	   		url: "<c:url value="/manager/lms/survey/lmsSurveyAjax.do"/>"
	   		, data: defaultParam
...
var gridEvent = {
	chkDelete : function() {
		...
		var result = confirm("선택한 설문을 삭제 하겠습니까?"); 
		
		if(result) {
			$.ajaxCall({
				url: "<c:url value="/manager/lms/survey/lmsSurveyDeleteAjax.do"/>"
	...
	chkCopy : function() {
		...
		var result = confirm("선택한 설문을 복사 하겠습니까?"); 
		
		if(result) {
			$.ajaxCall({
				url: "<c:url value="/manager/lms/survey/lmsSurveyCopyAjax.do"/>"
				, data: jQuery.param(checkedList)
				, success: function(data, textStatus, jqXHR){
...
function goWrite(modeVal,courseidVal){
	// 상세 tabpage
	var strMenuCd   = frmid + "_W";
	var strMenuText = "설문등록";
	var strLinkurl = "/manager/lms/survey/lmsSurveyWrite.do";
	// 탭이동시 열려있으면 닫고 이동한다.
	if(window.parent.g_managerLayerMenuId.sessionOncourseid != null && window.parent.g_managerLayerMenuId.sessionOncourseid != courseidVal){
		window.parent.closeTabMenu(strMenuCd);
	}
	// 전역변수 처리
	window.parent.g_managerLayerMenuId.sessionOncourseid  = courseidVal;
	...
	var item = { 
		...
		, callFn   : 'reloadFrame' 
		, funcData : ''
		, urlParam : "inputtype="+modeVal+"&courseid="+courseidVal+"&prefrmId="+frmid
	};
...
<body class="bgw">
	...
	<div class="contents_title clear" style="padding-top:60px;">
		<div class="fl">
			...
			<a href="javascript:;" id="deleteButton" class="btn_green" style="vertical-align:middle">삭제</a>
		...
		<div class="fr">
			<a href="javascript:;" id="copyButton" class="btn_green">선택설문 복사</a>

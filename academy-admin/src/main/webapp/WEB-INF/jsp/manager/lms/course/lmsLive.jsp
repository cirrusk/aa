...
$(document.body).ready(function(){
	...
	$("#aExcdlDown").on("click", function(){
		...
		if(result) {
			...
			postGoto("<c:url value="/manager/lms/course/lmsLiveExcelDownload.do"/>", defaultParam);
			hideLoading();
		}
	});	
});
...
var list = {
		...
		*/
		init : function() {
			...
			$.ajaxCall({
		   		url: "<c:url value="/manager/lms/course/lmsLiveListAjax.do"/>"
		   		, data: defaultParam
...
var gridEvent = {
	chkDelete : function() {
		...
		if(result) {
			$.ajaxCall({
				url: "<c:url value="/manager/lms/course/lmsLiveDeleteAjax.do"/>"
...
function goWrite(mode, courseid){
	...
	var strMenuText = "라이브과정등록";
	var strLinkurl = "/manager/lms/course/lmsLiveWrite.do";
	// 탭이동시 열려있으면 닫고 이동한다.
	if(window.parent.g_managerLayerMenuId.sessionOnLivecourseid != null && window.parent.g_managerLayerMenuId.sessionOnLivecourseid != courseid){
		window.parent.closeTabMenu(strMenuCd);
	}
	// 전역변수 처리
	window.parent.g_managerLayerMenuId.sessionOnLivecourseid  = courseid;
	...
	//본창에서 등록
	location.href="/manager/lms/course/lmsLiveWrite.do?mode="+mode+"&courseid="+courseid;
	return;
	//팝업창등록
	var param = {
			mode : mode ,
	...
	var popParam = {
			url : "<c:url value="/manager/lms/course/lmsLiveWrite.do" />"
			, width : 1024
			, height : 800
			, maxHeight : 800
			, params : param

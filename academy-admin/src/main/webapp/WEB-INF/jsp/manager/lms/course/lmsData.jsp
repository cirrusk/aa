	...
	$("#aExcdlDown").on("click", function(){
		...
		if(result) {
			...
			var initParam = {
				...
				, searchdatatype :$("#searchdatatype").val()
			...
			postGoto("<c:url value="/manager/lms/course/lmsDataExcelDownload.do"/>", defaultParam);
			hideLoading();
		}
	});	
});
	...
	*/
	init : function() {
		...
		var _colGroup = [
			...
			, {key:"categorytreename", label:"자료분류", width:"200", align:"center", sort:false}
			, {key:"coursename", label:"교육자료명", width:"500", align:"center", sort:false, formatter: function () {
				return "<a href=\"javascript:;\" onclick=\"javascript:goWrite('U', '" + this.item.courseid + "');\">"+this.item.coursename+"</a>";
			}}
			, {key:"datatypename", label:"자료유형", width:"100", align:"center", sort:false}
		...
		$.ajaxCall({
	   		url: "<c:url value="/manager/lms/course/lmsDataListAjax.do"/>"
	   		, data: defaultParam
...
var gridEvent = {
	chkDelete : function() {
		...
		for(var i = 0; i<listIdx.length;i++) {
			if( listIdx[i].item.openflag != "N") {
				if( !confirm(listIdx[i].item.coursename + " 자료가 공개 설정 되어 있습니다.\n삭제하시겠습니까?")){
					return;	
				}
			}
		}
		for(var i = 0; i<listIdx.length;i++) {
			if( listIdx[i].item.unitcount != "0") {
				if( !confirm(listIdx[i].item.coursename + " 자료가 정규과정에 설정 되어 있습니다.\n삭제하시겠습니까?")){
					return;	
				}
			}
		}
		...
		var result = confirm("선택한 교육자료를 삭제 하겠습니까?"); 
		
		if(result) {
			$.ajaxCall({
				url: "<c:url value="/manager/lms/course/lmsDataDeleteAjax.do"/>"
	...
	chkCopy : function() {
		...
		var len = checkedList.length;
		if(len == 0){
			alert("선택된 교육자료가 없습니다.");
			return;
		}
		var result = confirm("선택한 교육자료를 복사 하겠습니까?"); 
		
		if(result) {
			$.ajaxCall({
				url: "<c:url value="/manager/lms/course/lmsDataCopyAjax.do"/>"
				, data: jQuery.param(checkedList)
				, success: function(data, textStatus, jqXHR){
					alert(data.cnt + "건이 복제 되었습니다.");
					list.doSearch();
				},
				error: function( jqXHR, textStatus, errorThrown) {
		           	alert("<spring:message code="errors.load"/>");
...
function goWrite(mode, courseid){
	...
	var strMenuText = "교육자료등록";
	var strLinkurl = "/manager/lms/course/lmsDataWrite.do";
	// 탭이동시 열려있으면 닫고 이동한다.
	if(window.parent.g_managerLayerMenuId.sessionOnDatacourseid != null && window.parent.g_managerLayerMenuId.sessionOnDatacourseid != courseid){
		window.parent.closeTabMenu(strMenuCd);
	}
	// 전역변수 처리
	window.parent.g_managerLayerMenuId.sessionOnDatacourseid  = courseid;
	...
	<div class="tbl_write">
		<table id="tblSearch" width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				<th>자료유형</th>
				<td colspan="3">
					<select id="searchdatatype" name="searchdatatype" style="width:auto; min-width:100px" >
						<option value="">선택</option>
						<c:forEach items="${dataTypeList}" var="items">
	...
	<div class="contents_title clear">
		...
		<div class="fr">
			<div style="float:right;">
				<a href="javascript:;" id="copyButton" class="btn_green">교육자료 복사</a>
